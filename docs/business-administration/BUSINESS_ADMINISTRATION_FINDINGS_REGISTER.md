...
| **BA-F-006** | **`BusinessConfigurationContext` realtime sync incomplete** — client sends WebSocket messages; server comment: endpoints not implemented | Platform Engineering | Work tab / workspace module list may stale after admin changes; violates product sync requirement for installed/enabled business modules (see `APPLICATION_LIFECYCLE.md`; historical note archived under `docs/archive/session-summaries/business-workspace/businessWorkspaceArchitecture.md`) | BA-1A: server broadcast contract; BA-1D: integration test; subscribe rooms in `chatSocketService` or dedicated config channel | **G8 PARTIAL** | **BA-1A** (contract) + **BA-1D** (verify) |
