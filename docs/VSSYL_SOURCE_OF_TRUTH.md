...
4. **UX standards (constitutional framework):** [`docs/ux/UX_CONSTITUTION.md`](ux/UX_CONSTITUTION.md) — design tokens, layouts, components, accessibility; tokens in [`docs/ux/DESIGN_TOKENS.md`](ux/DESIGN_TOKENS.md) and `web/src/styles/tokens.css`
5. **Cursor rules must follow both** — start with **`.cursor/rules/source-of-truth.mdc`** and **`docs/VSSYL_SOURCE_OF_TRUTH.md`** (this file); agent rules are short files under **`.cursor/rules/`** (see **`RULES_SUMMARY.md`**). Long architecture notes live in **`docs/architecture/`**; UX standards in **`docs/ux/`**; how-to guides in **`docs/guides/`**.
6. **Outdated docs should be archived, not deleted**
7. **Implementation status must be updated after major changes**
