...
| F-051 | 2026-04-14 | Medium   | Session/auth UX reliability                       | **Partially mitigated (2026-04-17):** **`web/src/middleware.ts`** uses NextAuth **`withAuth`** for **`/admin-portal/*`**: unauthenticated → sign-in; authenticated non-**`ADMIN`** → **`/forbidden`**. **`admin-portal/layout.tsx`** aligns (non-admin → `/forbidden`). **`/forbidden`** page added. Remaining: profile / business shell sign-in UX and broader middleware coverage are unchanged.                                                                                                                                                                                                                                                                                    | `web/src/middleware.ts`, `web/src/app/forbidden/page.tsx`, `web/src/app/admin-portal/layout.tsx`, `web/src/app/profile/page.tsx`                                                                                                        | Open      | AI agent |
...
| A-046 | F-051             | Define a consistent protected-route UX strategy for the frontend, including when auth is enforced server-side, when client redirects are acceptable, and how unauthorized vs unauthenticated users should be presented.                                                                     | Medium   | In progress | Admin portal: middleware + `/forbidden` + layout. Extend pattern to other protected trees after review. Manual auth QA recommended.                                                                                                                                                                                                                                                                                                                                   |
