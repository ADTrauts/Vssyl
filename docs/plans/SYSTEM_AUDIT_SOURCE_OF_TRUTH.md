...
| F-043 | 2026-04-14 | Medium   | Validation consistency                            | **Partially mitigated (2026-04-17 / 2026-04-18 / 2026-04-15):** Shared **`validate`** + **`express-validator`** on **`pricing`**, **`contentReports`**, **`notification`**, **`ai-provider-usage`**, **`billing`**, **`member`**, plus **`scheduling`** (UUID **`id`** params on entity routes; **`POST /me/availability`** body/query for availability; removed per-request debug **`console.log`** middleware from **`scheduling`** router) and **`business`** (create/update/invite/member/logo path UUIDs and bodies). **`drive`**: JWT on **`GET /items/:itemId/activity`** and **`GET /shared`**; UUID **`itemId`** on activity. **`calendar`**: UUID path params; query validation for list/search/conflicts/free-busy/export; bodies for create calendar, auto-provision, create event, RSVP, import ICS, comments; public **`/rsvp`** query; consolidated imports; removed duplicate **`GET /freebusy`**. **`chat`**: UUID params; **`POST /messages/:messageId/read`** aligned with client; **`createThread`** uses route **`conversationId`**; **`reactions`** **`emoji`** body. Remaining: deeper **`scheduling`** POST bodies (**`/ai/*`**, admin creates); optional nested **`drive`** **`/files`** / **`/folders`**. | `server/src/middleware/validateRequest.ts`, `server/src/routes/pricing.ts`, `server/src/routes/contentReports.ts`, `server/src/routes/notification.ts`, `server/src/routes/ai-provider-usage.ts`, `server/src/routes/billing.ts`, `server/src/routes/member.ts`, `server/src/routes/scheduling.ts`, `server/src/routes/business.ts`, `server/src/routes/dashboard.ts`, `server/src/routes/widget.ts`, `server/src/routes/drive.ts`, `server/src/routes/calendar.ts`, `server/src/routes/chat.ts` | Partially mitigated | AI agent |
...
2. **Wave P — Platform entrypoints (Phase 5)**
  `A-047`, ~~`A-048`~~, ~~`A-049`~~, `A-050` (in progress — see **`F-056`**) (Stripe/webhooks, realtime, notification client, billing lifecycle).
...
| A-038 | F-043             | Adopt a consistent validation standard for backend routes: apply shared request validation at route boundaries for critical endpoints and reduce controller-local shape parsing to business-rule checks only.                                                                               | Medium   | In progress | **`drive`**, **`calendar`**, **`chat`** route validation added (**`F-043`**). Next: **`scheduling`** AI/admin POST payloads; optional nested **`drive`** file/folder routes. **`pnpm exec tsc --noEmit`** verified in **`server`**.                                                                                                                                                                                                                                                                                                                                                  |
| A-039 | F-044             | Normalize backend error handling onto structured logging for both development and production, and stop returning raw internal error text except where explicitly intended and safe.                                                                                                         | High     | In progress | **`logger.error`** on unhandled errors; prod **5xx** generic **`message`**; scheduling per-request **`console`** gated to **`NODE_ENV !== 'production'`**; **`PUT /api/profile`** errors → **`logger`**; **migrations, cron jobs, listen/bootstrap fatal** → **`logger`**. **Remaining:** **`console`** only when **`logger`** throws (registration/login); optional tests for prod error JSON shape.                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| A-050 | F-056             | Consolidate module billing and Stripe lifecycle handling behind one authoritative service path, use real Stripe product/price mapping, and ensure webhook processing updates both platform and module subscription state consistently.                                                      | High     | In progress | **`PaymentService.handleWebhookEvent`** delegates to **`StripeService`** (no duplicate switch). **`payment_intent.succeeded`** with **`metadata.type === 'module_subscription'`** runs via **`StripeService`** → **`PaymentService.completeModuleSubscriptionFromPaymentIntent`**. **`ModuleSubscriptionService`** still runs after core handlers. Tests: **`server/src/services/__tests__/stripeWebhookBilling.test.ts`** (delegation + **`moduleSubscription.updateMany`** for invoice / subscription deleted). **Remaining:** manual Stripe CLI validation; optional E2E.                                                                                                                                                                                                                                                                                                                                                                                                                        |
