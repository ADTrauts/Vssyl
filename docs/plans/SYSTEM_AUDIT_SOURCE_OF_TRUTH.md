...
| --- | --- | --- | --- | --- | --- | --- | --- |
| F-001 | 2026-04-14 | Critical | Auth / Admin setup | `server/src/routes/admin-setup.ts` exposes unauthenticated account bootstrap and admin-promotion endpoints, including creation/promotion of an admin user and returning a plaintext default password in API responses. Mounted at `/api/admin-setup` in `server/src/index.ts` without `authenticateJWT`. | `server/src/routes/admin-setup.ts`, `server/src/index.ts` | Open | AI agent |
| F-002 | 2026-04-14 | Critical | Billing / access control | `server/src/routes/debug-business-tier.ts` explicitly allows unauthenticated business tier inspection and mutation. The `POST /api/debug/business-tier/:businessId` endpoint can change `business.tier` without auth and is mounted publicly in `server/src/index.ts`. | `server/src/routes/debug-business-tier.ts`, `server/src/index.ts` | Open | AI agent |
| F-003 | 2026-04-14 | Critical | User data exposure | `GET /api/debug/users` in `server/src/index.ts` is publicly exposed and returns user identifiers and profile metadata (`id`, `email`, `name`, `userNumber`, `role`, verification state, timestamps) with no authentication. | `server/src/index.ts` | Open | AI agent |
| F-004 | 2026-04-14 | High | Authorization bypass | `server/src/routes/ai-centralized.ts` defines `requireAdmin` so that any authenticated request with `req.user?.id` is allowed through immediately, making the intended admin check unreachable for normal authenticated users. Admin-only centralized AI endpoints are therefore not actually admin-only. | `server/src/routes/ai-centralized.ts`, `server/src/index.ts`, `server/src/middleware/auth.ts` | Open | AI agent |
| F-005 | 2026-04-14 | Medium | Auth consistency / security drift | Alternate JWT middleware exists outside the canonical auth path. `server/src/routes/ai-context-debug.ts` verifies against `process.env.JWT_SECRET || 'fallback-secret'` and trusts token role claims directly; `server/src/routes/businessAI.ts` also trusts decoded token payloads instead of loading the database-backed user used by `server/src/middleware/auth.ts`. | `server/src/routes/ai-context-debug.ts`, `server/src/routes/businessAI.ts`, `server/src/middleware/auth.ts` | Open | AI agent |
| F-006 | 2026-04-14 | High | Admin surface protection | `/api/admin/logs/*` is mounted behind authentication in `server/src/index.ts`, but `server/src/routes/admin-logs.ts` and `server/src/controllers/logController.ts` show no admin-only authorization check. This appears to expose log viewing/export/retention operations to any authenticated user, despite the route family being admin-named and operationally sensitive. | `server/src/index.ts`, `server/src/routes/admin-logs.ts`, `server/src/controllers/logController.ts` | Open | AI agent |
| F-007 | 2026-04-14 | Medium | Information disclosure | `server/src/routes/debug-modules.ts` intentionally exposes module inventory, AI context registration counts, and per-module manifest data without authentication. It is publicly mounted under `/api/debug` in `server/src/index.ts`. | `server/src/routes/debug-modules.ts`, `server/src/index.ts` | Open | AI agent |
| F-008 | 2026-04-14 | Medium | Information disclosure | `GET /api/schema` in `server/src/routes/health.ts` is public and enumerates database tables from `information_schema`, including a location-table existence check. This is not destructive, but it unnecessarily exposes internal schema details on a public health surface. | `server/src/routes/health.ts`, `server/src/index.ts` | Open | AI agent |
| F-009 | 2026-04-14 | Medium | Authorization inconsistency | `server/src/routes/admin-fix-subscriptions.ts` is mounted behind `authenticateJWT`, but its `GET /api/admin/fix-subscriptions/check` endpoint does not enforce admin role while the corresponding mutation endpoint does. This exposes internal schema metadata to any authenticated user and indicates inconsistent privilege enforcement in emergency/admin maintenance routes. | `server/src/routes/admin-fix-subscriptions.ts`, `server/src/index.ts` | Open | AI agent |
| F-010 | 2026-04-14 | Critical | Tenant isolation / authorization | `server/src/routes/org-chart.ts` applies authentication globally but the CRUD handlers shown do not enforce business membership, role, or permission checks for org-chart data. The corresponding `server/src/services/orgChartService.ts` methods shown perform direct Prisma CRUD by `businessId`/entity id with no authorization guardrails. This creates a likely cross-tenant write/read risk if any authenticated user can call these endpoints with another business ID. | `server/src/routes/org-chart.ts`, `server/src/services/orgChartService.ts`, `server/src/index.ts` | Open | AI agent |
| F-011 | 2026-04-14 | High | Payment integration correctness | `server/src/routes/payment.ts` documents `POST /webhook` as unauthenticated Stripe webhook intake, but `server/src/index.ts` mounts the entire `/api/payment` router behind `authenticateJWT`. That means external Stripe callbacks will be rejected before `handleWebhook` runs unless there is an out-of-band path not shown here. | `server/src/routes/payment.ts`, `server/src/index.ts`, `server/src/middleware/auth.ts` | Open | AI agent |
| F-012 | 2026-04-14 | High | Information disclosure / operational exposure | `server/src/routes/debug-database.ts` exposes unauthenticated HR migration/table probing under `/api/debug/database/hr-tables`. It returns table existence state and raw error details for missing tables, which creates another public operational debug surface. | `server/src/routes/debug-database.ts`, `server/src/index.ts` | Open | AI agent |
| F-013 | 2026-04-14 | Medium | Auth wiring / feature correctness | `server/src/routes/googleOAuth.ts` mounts business auth-url, test-config, and status endpoints without `authenticateJWT`, while their controller methods expect `req.user` to exist and return `401` when absent. Only the callback endpoint is naturally public. This is inconsistent auth wiring that likely leaves protected Google OAuth management endpoints permanently broken or vulnerable to future incorrect fixes. | `server/src/routes/googleOAuth.ts`, `server/src/controllers/googleOAuthController.ts`, `server/src/index.ts` | Open | AI agent |
| F-014 | 2026-04-14 | Medium | Account enumeration | `POST /api/auth/resend-verification` in `server/src/index.ts` returns `404 User not found` for unknown emails and different responses for known-but-verified or known-unverified users. This allows straightforward account existence enumeration. | `server/src/index.ts` | Open | AI agent |
| F-015 | 2026-04-14 | Critical | Cross-user write / tenant isolation | `server/src/controllers/notificationController.ts` creates notifications using `userId` from the request body and does not constrain it to `req.user.id` or admin-only flows. Any authenticated caller reaching the route can create notifications for arbitrary users. | `server/src/controllers/notificationController.ts`, `server/src/routes/notification.ts` | Open | AI agent |
...
| F-018 | 2026-04-14 | High | Tenant scoping / provisioning integrity | `server/src/services/dashboardService.ts` accepts `businessId`, `institutionId`, and `householdId` for dashboard creation without proving the user belongs to those contexts. When `businessId` is supplied, `seedBusinessWorkspaceResources` can then create the business primary calendar and assign the caller as `OWNER` if one does not already exist. | `server/src/services/dashboardService.ts`, `server/src/services/businessWorkspaceSeeder.ts` | Open | AI agent |
| F-019 | 2026-04-14 | Medium | Membership lifecycle enforcement | `server/src/controllers/memberController.ts` checks business membership in some list/read flows using `findUnique({ businessId_userId })` but without requiring `isActive: true`. That means soft-removed members may still pass certain membership gates, unlike adjacent endpoints that explicitly check `member?.isActive`. | `server/src/controllers/memberController.ts` | Open | AI agent |
| F-020 | 2026-04-14 | Medium | Invitation redemption integrity | `server/src/controllers/businessController.ts` accepts a business invitation token and creates membership for the currently authenticated user, but the flow shown does not verify that `invitation.email` matches the authenticated account email. A valid token can therefore be redeemed by the wrong account. | `server/src/controllers/businessController.ts` | Open | AI agent |
| F-021 | 2026-04-14 | Medium | Marketplace isolation / unpublished data exposure | `server/src/controllers/moduleController.ts#getModuleDetails` fetches any module by id and returns `status`, `manifest`, dependencies, permissions, and review data without applying the `APPROVED`-only filter used by marketplace listing. This likely exposes non-public module metadata to any authenticated user who knows a module id. | `server/src/controllers/moduleController.ts` | Open | AI agent |
| F-022 | 2026-04-14 | Medium | Wrong-tenant context selection | `server/src/ai/services/ModuleAIContextService.ts` resolves a default `businessId` by taking the first active business membership (`joinedAt ASC`) when a business context is absent. For multi-business users, this can select the wrong tenant context for internal AI context fetches. | `server/src/ai/services/ModuleAIContextService.ts` | Open | AI agent |
| F-023 | 2026-04-14 | High | Context-binding integrity | Drive creation flows trust client-supplied context ids without proving ownership of the target dashboard/context. `createFolder` accepts `dashboardId` directly, and file upload accepts `folderId`/`dashboardId` directly, only overriding `folderId` for chat uploads. This allows metadata attachment to arbitrary dashboards/folders if the ids are known, even though the created records remain owned by the uploader. | `server/src/controllers/folderController.ts`, `server/src/controllers/fileController.ts` | Open | AI agent |
| F-024 | 2026-04-14 | Medium | Conversation scoping / participant policy | `server/src/controllers/chatController.ts#createConversation` accepts arbitrary `participantIds` and `dashboardId` from the client, ensures only that the current user is included, and creates the conversation directly. There is no visible validation that the dashboard belongs to the caller's context or that the requested participants are valid for that context. | `server/src/controllers/chatController.ts` | Open | AI agent |
| F-025 | 2026-04-14 | Medium | Context-binding integrity | `server/src/controllers/notesController.ts#createNote` trusts client-supplied `dashboardId`, `businessId`, and `folderId` and creates the note directly for `createdById = userId` without verifying that those context ids belong to the caller or are mutually consistent. This can create mis-scoped notes and undermine tenant/data organization guarantees. | `server/src/controllers/notesController.ts` | Open | AI agent |
| F-026 | 2026-04-14 | Critical | Tenant isolation / IDOR | `server/src/controllers/schedulingController.ts` reads and mutates schedules by `id` only in several admin paths (`getScheduleById`, `updateSchedule`, `deleteSchedule`, `cloneSchedule`, `getScheduleShifts`) after middleware authorizes a caller for some `businessId`. The handlers do not then tie the loaded schedule/shift back to the authorized business, so a user with scheduling access in one business can likely act on another business's schedules if ids are known. | `server/src/controllers/schedulingController.ts`, `server/src/middleware/schedulingPermissions.ts` | Open | AI agent |
| F-027 | 2026-04-14 | High | Tenant-binding integrity | `server/src/controllers/schedulingController.ts#createSchedule` and `createShift` trust `businessId` from the request body, while `server/src/middleware/schedulingPermissions.ts` authorizes based on `businessId` from query first and body second. This split trust model can authorize one tenant context while persisting rows for another tenant context if mismatched values are supplied. | `server/src/controllers/schedulingController.ts`, `server/src/middleware/schedulingPermissions.ts` | Open | AI agent |
| F-028 | 2026-04-14 | High | Tenant spoofing / context ownership | `server/src/controllers/calendarController.ts#createCalendar` accepts arbitrary `contextType` and `contextId` and immediately creates a calendar with the caller as `OWNER`, without verifying that the caller belongs to the referenced business or household context. | `server/src/controllers/calendarController.ts` | Open | AI agent |
| F-029 | 2026-04-14 | High | Cross-context linkage integrity | `server/src/controllers/placeMeetingController.ts#linkToCalendar` allows a meeting participant to supply `existingEventId` and link the meeting directly to that event without verifying access to the existing calendar event. Only the branch that creates a new event checks calendar membership. | `server/src/controllers/placeMeetingController.ts` | Open | AI agent |
| F-030 | 2026-04-14 | High | Cross-business IDOR | `server/src/controllers/placeListingController.ts` checks admin access for the `:businessId` in the route, but `updateInteractionLink` and `deleteInteractionLink` mutate `BusinessInteractionLink` rows by `linkId` only. If a caller knows another business's `linkId`, they can likely update or delete it while presenting their own business id in the URL. | `server/src/controllers/placeListingController.ts` | Open | AI agent |
| F-031 | 2026-04-14 | Critical | Tenant isolation / trusted body data | Org-chart employee assignment routes pass caller-supplied `businessId`, `userId`, `positionId`, and `assignedById` directly into `EmployeeManagementService` without route-level authorization checks, and the service loads the target position by `positionId` only before creating assignments using the caller-provided `businessId`. This creates a likely cross-tenant assignment and org-data integrity risk. | `server/src/routes/org-chart.ts`, `server/src/services/employeeManagementService.ts` | Open | AI agent |
| F-032 | 2026-04-14 | Medium | Context filter mismatch | `server/src/controllers/activityFeedController.ts` accepts `dashboardId` but does not apply it in any of the source queries. The endpoint aggregates by user only, so a caller requesting dashboard-scoped activity can receive mixed cross-dashboard activity instead of the requested context. | `server/src/controllers/activityFeedController.ts` | Open | AI agent |
| F-033 | 2026-04-14 | Medium | Cross-user directory exposure | `server/src/controllers/searchController.ts` includes a member search provider that returns user results from the global user table by name/email match, excluding only the caller. It is not scoped by tenant, relationship, or business membership, so any authenticated user can discover other users and their email addresses. | `server/src/controllers/searchController.ts` | Open | AI agent |
| F-034 | 2026-04-14 | Medium | Business summary scoping error | `server/src/services/permissionService.ts#getBusinessPermissionSummary` counts positions and employees by `businessId`, but its permission summary queries (`permission.findMany(distinct moduleId)` and `permission.groupBy(category)`) are not filtered by `businessId`. That means a business-scoped summary can include global permission data from other tenants. | `server/src/services/permissionService.ts` | Open | AI agent |
| F-035 | 2026-04-14 | Critical | IDOR / cross-user mutation | `server/src/controllers/todoController.ts#completeTask` and `reopenTask` update tasks by `id` only and do not verify that the caller created the task or is the assignee before changing state. Unlike `getTaskById` and `deleteTask`, these endpoints lack ownership/access scoping. | `server/src/controllers/todoController.ts` | Open | AI agent |
| F-036 | 2026-04-14 | Critical | Cross-user data exposure | `server/src/controllers/todoController.ts#getTasksForConversation` calls `TodoChatIntegrationService.getTasksForConversation`, which loads broadly matching tasks without scoping by `userId` or `conversationId` and then returns all fetched rows. The endpoint also does not verify that the caller belongs to the requested conversation. | `server/src/controllers/todoController.ts`, `server/src/services/todoChatIntegrationService.ts` | Open | AI agent |
| F-037 | 2026-04-14 | High | Project-scoping IDOR | To-Do project APIs trust `dashboardId` for listing/creation and then update/delete projects by `id` only, with no check that the caller owns or may access the target dashboard/project. Because `TaskProject` stores context ids but no owner, this creates likely cross-context listing and modification risk. | `server/src/controllers/todoController.ts` | Open | AI agent |
| F-038 | 2026-04-14 | High | Context-binding integrity | `server/src/controllers/todoController.ts#createTask` and `TodoChatIntegrationService.createTaskFromMessage` trust client-supplied `dashboardId`, `businessId`, and `householdId` without revalidating them, and the chat-integrated create path also does not verify that the caller belongs to the referenced conversation before creating the task. | `server/src/controllers/todoController.ts`, `server/src/services/todoChatIntegrationService.ts` | Open | AI agent |
| F-039 | 2026-04-14 | Medium | Context-binding integrity | `server/src/controllers/notesFolderController.ts#createFolder` trusts client-supplied `dashboardId`, `businessId`, and `parentId`, and `updateFolder` allows parent reassignment by connecting `parentId` without verifying that the target parent folder belongs to the same caller/context. | `server/src/controllers/notesFolderController.ts` | Open | AI agent |
...
| F-047 | 2026-04-14 | High | Frontend architecture / shared shell hotspot | `web/src/app/dashboard/DashboardLayout.tsx` has become a frontend god component that acts as the shared shell for much of the product, mixing personal dashboard chrome, work-tab behavior, business configuration, household creation/invites, sidebars, routing fallbacks, and module icon registries in one file. That coupling is already leaking across layers: `web/src/components/sidebar/RightSidebarCustomizer.tsx` imports `MODULE_ICONS` directly from the dashboard route module. This makes shell changes high-risk and increases the cost of any future layout split or sidebar refactor. | `web/src/app/dashboard/DashboardLayout.tsx`, `web/src/components/sidebar/RightSidebarCustomizer.tsx` | Open | AI agent |
...
| F-049 | 2026-04-14 | High | Frontend API contract / proxy drift | Some frontend pages still bypass the established Next.js proxy/path conventions in ways that are likely incorrect. `web/src/app/profile/page.tsx` calls `${API_BASE_URL}/profile` for authenticated profile reads/updates instead of the expected `/api/profile` path, and `web/src/app/admin-portal/test-api/page.tsx` calls `${API_BASE_URL}/admin-portal/test` instead of `/api/admin-portal/test`. These are concrete examples of frontend/backend contract drift on top of the broader proxy standard. | `web/src/app/profile/page.tsx`, `web/src/app/admin-portal/test-api/page.tsx`, `web/src/app/api/[...slug]/route.ts` | Open | AI agent |
| F-050 | 2026-04-14 | Medium | Frontend API consistency / debug leakage | Frontend API usage is not consistently routed through the same browser-side abstraction. Some export flows call the backend directly with full environment URLs (`calendar.ts` ICS export, `logs.ts` export), and several module/context files still contain hardcoded localhost debug-ingest calls to `127.0.0.1:7242`. Even when some of these paths are intentional or non-critical, they increase network-behavior drift, make browser/API debugging harder, and risk shipping development-only telemetry noise into normal runtime paths. | `web/src/api/calendar.ts`, `web/src/api/logs.ts`, `web/src/contexts/GlobalTrashContext.tsx`, `web/src/components/modules/ChatModule.tsx`, `web/src/components/modules/CalendarModule.tsx`, `web/src/components/modules/DriveModule.tsx` | Open | AI agent |
| F-051 | 2026-04-14 | Medium | Session/auth UX reliability | Frontend route protection is inconsistent and relies heavily on client-side redirects instead of a single predictable gating strategy. There is no repo-level `middleware.ts`; `web/src/app/admin-portal/layout.tsx` performs its admin check on the client and redirects non-admin users to the login page rather than an explicit forbidden state; and some authenticated pages such as `web/src/app/profile/page.tsx` simply render a signed-out message instead of following the redirect pattern used elsewhere. This weakens consistency for protected-route UX and makes auth behavior harder to reason about across route trees. | `web/src/app/admin-portal/layout.tsx`, `web/src/app/profile/page.tsx`, `web/src/app/business/[id]/workspace/layout.tsx`, `web/src/app/layout.tsx` | Open | AI agent |
| F-052 | 2026-04-14 | Critical | Billing / webhook intake correctness | The Stripe webhook entrypoint is effectively broken as wired. `server/src/index.ts` mounts the entire `/api/payment` router behind `authenticateJWT`, which blocks Stripe's unauthenticated callbacks, and global `express.json()` runs before `paymentController.handleWebhook`, even though `stripe.webhooks.constructEvent` requires the raw request body for signature verification. This makes the main billing event-ingest path unreliable before any downstream business logic is considered. | `server/src/index.ts`, `server/src/middleware/auth.ts`, `server/src/controllers/paymentController.ts` | Open | AI agent |
| F-053 | 2026-04-14 | High | Real-time delivery architecture | `server/src/services/chatSocketService.ts` keeps a single in-memory `userId -> socketId` mapping for targeted emits. The newest connection replaces the previous one, so only the last tab/device is guaranteed to receive user-targeted notification or direct events. Because the registry is process-local, this also does not provide correct delivery semantics across multiple server instances. | `server/src/services/chatSocketService.ts` | Open | AI agent |
| F-054 | 2026-04-14 | High | Real-time authorization / privacy | Socket room and presence behavior is weaker than the HTTP layer. `chatSocketService` allows arbitrary `join_conversation`, `join_business`, and `join_schedule` room subscriptions without verifying membership for the supplied IDs, and `handlePresenceUpdate` broadcasts presence changes to all connected users globally. This creates cross-tenant realtime exposure risk and an overly broad presence surface. | `server/src/services/chatSocketService.ts` | Open | AI agent |
| F-055 | 2026-04-14 | High | Notification realtime client lifecycle | The notification realtime client is prone to duplicate listeners and unreliable connection state. `web/src/lib/notificationSocket.ts` returns a new object each render while tracking `isConnected` in a mutable ref, and consumers such as `web/src/components/NotificationBadge.tsx` subscribe inside `useEffect([notificationSocket])` without corresponding unsubscribe logic. This makes notification counts and client-side realtime behavior susceptible to duplicate event handling and stale connection state. | `web/src/lib/notificationSocket.ts`, `web/src/components/NotificationBadge.tsx`, `web/src/lib/websocketUtils.ts` | Open | AI agent |
...
| F-059 | 2026-04-14 | Critical | Verification discipline / CI enforcement | The repo’s automated checks are not wired into a release-grade verification pipeline. Root `package.json` defines `test` as Playwright-only, `server/package.json` contains the Vitest suite, but `.github/workflows/ci.yml` runs only lint/format/type-check and does not execute Vitest or Playwright. This means meaningful automated tests can exist without protecting merges or deployments. | `package.json`, `server/package.json`, `.github/workflows/ci.yml`, `memory-bank/testingStrategy.md` | Open | AI agent |
| F-060 | 2026-04-14 | High | Deployment safety / migration gating | Production startup tolerates migration failure and still starts the server. In `server/src/index.ts`, schema build and `prisma migrate deploy` failures are logged, but the process intentionally continues and reconnects Prisma instead of failing fast. This allows a deployment to appear alive while the database schema may be out of sync with application code. | `server/src/index.ts` | Open | AI agent |
| F-061 | 2026-04-14 | High | Health/readiness signal mismatch | The deployed health signal is shallower than it appears. `server/src/index.ts` defines a simple `/api/health` route before the richer health router, and `server/Dockerfile.production` uses that endpoint for the container healthcheck. As a result, the container can be marked healthy without proving database-backed readiness from the deeper health handler. | `server/src/index.ts`, `server/src/routes/health.ts`, `server/Dockerfile.production` | Open | AI agent |
| F-062 | 2026-04-14 | High | CI/repo configuration drift | The configured CI pipeline appears out of sync with the current workspace layout and toolchain. `.github/workflows/ci.yml` uses `pnpm` 8 while the repo and Dockerfiles pin `pnpm@10.11.0`, runs `format:check` for `shared` even though `shared/package.json` does not define that script, and references a `prisma` workspace package from `pnpm-workspace.yaml` even though there is no `prisma/package.json`. This creates a high risk that CI either fails for configuration reasons or no longer reflects the real repo contract. | `.github/workflows/ci.yml`, `package.json`, `shared/package.json`, `pnpm-workspace.yaml`, `server/Dockerfile.production` | Open | AI agent |
...
| F-064 | 2026-04-14 | Medium | Release operations / rollback readiness | Deployment automation favors speed over staged release safety and operator guidance. `cloudbuild.yaml` builds and deploys directly without test gates or gradual rollout controls, and there is no clear rollback or deployment runbook under `docs/` covering Cloud Run revision rollback, failed-migration response, or release recovery procedures. | `cloudbuild.yaml`, `docs/`, `memory-bank/testingStrategy.md` | Open | AI agent |
...
| A-004 | F-004 | Replace local `requireAdmin` in `ai-centralized` with canonical role enforcement and verify all sensitive centralized AI endpoints require the intended privilege level. | High | Backlog | Focused backend tests, route-level validation |
| A-005 | F-005 | Normalize JWT/auth handling for `ai-context-debug` and `businessAI` onto the canonical `authenticateJWT` flow or a shared equivalent that loads DB-backed user state. | Medium | Backlog | Focused backend tests, manual API verification |
| A-006 | F-006 | Require explicit admin authorization for `/api/admin/logs/*` and validate log export, retention, and analytics endpoints against intended admin-only policy. | High | Backlog | Focused backend tests, route-level validation |
| A-007 | F-007, F-008 | Remove or environment-gate public debug and schema enumeration endpoints that expose internal module or database metadata. | Medium | Backlog | Route-level validation |
| A-008 | F-009 | Normalize privilege checks across maintenance/admin fix routes so read and write operations use the same intended authorization standard. | Medium | Backlog | Focused backend tests |
...
| A-011 | F-012 | Remove or environment-gate database debug routes and eliminate raw schema/migration error leakage from public responses. | High | Backlog | Route-level validation |
| A-012 | F-013 | Fix Google OAuth route auth wiring so protected management/status endpoints use authenticated business-member checks while the callback remains public only for the OAuth exchange. | Medium | Backlog | Focused backend tests, manual API verification |
| A-013 | F-014 | Normalize resend-verification responses to avoid revealing whether an email exists or is already verified. | Medium | Backlog | Route-level validation |
| A-014 | F-015 | Restrict notification creation so callers can only create notifications for themselves unless a dedicated trusted/admin flow is used. Add tests for cross-user notification injection. | Critical | Done | `POST /api/notifications` always uses `req.user.id`; body `userId` ignored. Admin path: `POST /api/notifications/for-user` + role `ADMIN`. |
...
| A-016 | F-018 | Require explicit membership/ownership validation before creating context-bound dashboards or seeding business workspace resources, especially before assigning calendar ownership. | High | Backlog | Focused backend tests, route-level validation |
| A-017 | F-019 | Normalize business membership checks to require `isActive` consistently across read, invitation, and member-management endpoints. | Medium | Backlog | Focused backend tests |
| A-018 | F-020 | Bind invitation acceptance to the invited email identity or otherwise enforce the intended invite-recipient model. | Medium | Backlog | Focused backend tests, manual API verification |
| A-019 | F-021 | Apply publication/visibility rules consistently to module detail access, or explicitly define which unpublished metadata authenticated users may view. | Medium | Backlog | Route-level validation |
| A-020 | F-022 | Stop inferring default business context by oldest membership for AI context fetches; require explicit tenant context or a safer active-context resolution strategy. | Medium | Backlog | Focused backend tests, manual AI-context validation |
| A-021 | F-023, F-025 | Revalidate `dashboardId`, `folderId`, `businessId`, and similar context ids on server-side create flows before persisting records. Prefer deriving context from authorized parent objects where possible. | High | Backlog | Focused backend tests, route-level validation |
| A-022 | F-024 | Enforce dashboard/context ownership and participant eligibility rules when creating conversations, then add tests for invalid dashboard and participant combinations. | Medium | Backlog | Focused backend tests |
| A-023 | F-026, F-027 | Bind scheduling reads and mutations to the authorized `businessId` end-to-end, and reconcile route middleware context with persisted `businessId` so body/query mismatches cannot create cross-tenant records. | Critical | Backlog | Focused backend tests, route-level validation |
| A-024 | F-028 | Require membership/ownership validation before creating calendars in business or household contexts, and re-check auto-provisioning flows against the same rule. | High | Backlog | Focused backend tests |
| A-025 | F-029 | Verify access to existing calendar events before linking Place meetings to them, not just when creating a new event. | High | Backlog | Focused backend tests, manual API verification |
| A-026 | F-030 | Scope Place interaction-link updates/deletes through business-owned listings rather than `linkId` alone. | High | Backlog | Focused backend tests, route-level validation |
| A-027 | F-031 | Add explicit business membership/manager authorization to org-chart employee assignment routes and ensure position/business consistency is enforced inside the service layer. | Critical | Done | Same routes + `employeeManagementService` position/business checks. Integration test covers `POST /employees/validate` manage gate (403 for non-manager member). |
| A-028 | F-032 | Either honor `dashboardId` in activity feed queries or remove the parameter and document the endpoint as intentionally cross-dashboard. | Medium | Backlog | Focused backend tests |
| A-029 | F-033 | Define and enforce the intended visibility rules for member search results, then scope directory search accordingly. | Medium | Backlog | Route-level validation |
| A-030 | F-034 | Apply `businessId` scoping consistently inside business-scoped permission summary queries. | Medium | Backlog | Focused backend tests |
| A-031 | F-035 | Add the same ownership/assignee access checks to task state transitions that already exist for task reads/deletes, and verify unauthorized task IDs cannot be completed or reopened. | Critical | Backlog | Focused backend tests, route-level validation |
| A-032 | F-036 | Replace conversation-task lookup with a properly scoped link model or a query constrained by authorized conversation membership and task ownership/context. | Critical | Backlog | Focused backend tests |
| A-033 | F-037 | Introduce explicit access rules for TaskProject resources and scope list/create/update/delete operations through authorized dashboards or business contexts. | High | Backlog | Focused backend tests, route-level validation |
| A-034 | F-038, F-039 | Revalidate To-Do and Notes-folder context ids server-side before creating or moving records, and ensure chat-derived task creation proves conversation membership. | High | Backlog | Focused backend tests |
...
| A-042 | F-047 | Split `DashboardLayout` into smaller shell components/hooks and move shared registries/constants (such as module icon mappings) out of route modules into stable shared frontend modules. | High | Backlog | Focused frontend tests, manual QA for dashboard/sidebar flows |
...
| A-044 | F-049 | Audit frontend API paths for proxy adherence and fix concrete path-contract drift first, starting with authenticated profile and admin test surfaces. | High | Backlog | Focused frontend tests, route-level/API validation |
| A-045 | F-050 | Centralize blob/export and debug-only network helpers so direct backend calls and localhost-only telemetry are explicitly gated, documented, and removed from normal runtime paths where unnecessary. | Medium | Backlog | Focused frontend tests, manual browser verification |
| A-046 | F-051 | Define a consistent protected-route UX strategy for the frontend, including when auth is enforced server-side, when client redirects are acceptable, and how unauthorized vs unauthenticated users should be presented. | Medium | Backlog | Focused frontend tests, manual auth-flow QA |
...
| A-048 | F-053, F-054 | Redesign realtime delivery semantics to support multiple simultaneous sockets per user, authorize room joins against tenant membership, and define a production-safe cross-instance broadcast strategy. | High | In progress | `server/src/services/chatSocketService.ts`: user-targeted emits use room `user_${userId}` (multi-tab); `join_conversation` / `join_business` / `join_schedule` and typing are membership-checked. Optional **`SOCKET_IO_REDIS_URL`** (fallback **`REDIS_URL`**) attaches `@socket.io/redis-adapter` before `listen` in `bootstrap()` for multi-instance broadcasts; missing or failing Redis logs and continues in-memory. Deeper room lifecycle refresh still backlog. |
| A-049 | F-055 | Replace ad hoc notification socket subscriptions with a stable shared client/store or hook contract that supports cleanup, accurate connection state, and one listener registration path per consumer. | High | Backlog | Focused frontend tests, manual notification QA |
...
| A-053 | F-059 | Make release-quality verification explicit at the repo root and in CI: run server automated tests in CI at minimum, define where E2E fits, and align the monorepo’s default `test` contract with the checks required before merge or deploy. | Critical | In progress | Root `test` runs server Vitest; `verify:ci` and GitHub Actions run `pnpm type-check` and `pnpm test`. Full-repo `pnpm lint` and Prettier `format:check` are deferred until backlog cleanup. E2E placement still to document. |
| A-054 | F-060, F-061 | Fail deploys when startup migrations or readiness checks do not establish a safe serving state. Separate liveness from readiness, and ensure health probes reflect database-backed readiness rather than shallow process availability. | High | In progress | Implemented fail-fast production startup migrations/readiness in `server/src/index.ts`, removed the shallow shadow `/api/health` route, and pointed the production Docker healthcheck at `/api/ready`. `pnpm type-check` passed locally. Production-safety review and deploy-path validation still pending. |
| A-055 | F-062 | Reconcile CI/workspace/tooling configuration with the actual repo state: align pnpm versions, remove or restore broken filtered-script targets, and ensure CI commands match real packages and scripts. | High | In progress | Aligned CI to `pnpm@10.11.0`, removed the nonexistent `prisma` workspace package from `pnpm-workspace.yaml`, and fixed root script / CI package filters to use real workspace package names. CI gates `type-check` + `test` only until eslint/prettier are brought in line; added `server/.prettierignore` (excludes `dist`) for future format runs. |
...
| A-057 | F-064 | Add an operational deployment/rollback runbook that covers Cloud Run revision rollback, failed startup migrations, webhook/billing incident response, and which checks must pass before a production deploy. | Medium | Backlog | Production-safety review, manual dry-run review |
...
3. `F-017`, `F-024`, `F-037`
   Tighten chat/project lifecycle and conversation-linked mutation paths.
...
1. `F-059`, `F-060`, `F-061`, `F-062`
   Fix the release gates and readiness contract first so deploy safety is enforceable.
2. `F-063`
