...
| F-054 | 2026-04-14 | High     | Real-time authorization / privacy                 | **Partially mitigated (2026-04-17):** `join_conversation`, `join_business`, `join_schedule`, and typing paths are **membership-gated** (see **F-016**). **Remaining:** `handlePresenceUpdate` still uses `this.io.emit('user_presence', …)` (global broadcast), which is broader than tenant-scoped chat rooms.                                                                                                                                                                                                                                                                                                                                                                      | `server/src/services/chatSocketService.ts`                                                                                                                                                                                              | Open      | AI agent |
...
| A-048 | F-053, F-054      | Redesign realtime delivery semantics to support multiple simultaneous sockets per user, authorize room joins against tenant membership, and define a production-safe cross-instance broadcast strategy.                                                                                     | High     | In progress | **`F-053` mitigated** in findings table. **`F-054`:** room joins gated; **remaining** — scope `presence_update` / `user_presence` broadcast (currently `io.emit` global).                                                                                                                                                                                                                     |
