...
| F-051 | 2026-04-14 | Medium   | Session/auth UX reliability                       | **Partially mitigated (2026-04-17):** Middleware gates **`/admin-portal/*`** (non-**`ADMIN`** → **`/forbidden`**) and **`/profile/*`** (unauthenticated → sign-in). **`/forbidden`** page; **`admin-portal/layout`** aligned. **`profile/page`** client fallback redirect to login with **`callbackUrl`**. Remaining: **`/business/[id]/workspace`** and other shells still rely on layout/client patterns only.                                                                                                                                                                                                                                                                       | `web/src/middleware.ts`, `web/src/app/forbidden/page.tsx`, `web/src/app/admin-portal/layout.tsx`, `web/src/app/profile/page.tsx`, `web/src/app/business/[id]/workspace/layout.tsx`                                                      | Open      | AI agent |
...
| A-046 | F-051             | Define a consistent protected-route UX strategy for the frontend, including when auth is enforced server-side, when client redirects are acceptable, and how unauthorized vs unauthenticated users should be presented.                                                                     | Medium   | In progress | Admin portal + **`/profile/*`** middleware; business workspace TBD. Manual auth QA recommended.                                                                                                                                                                                                                                                                                                                                                                      |
