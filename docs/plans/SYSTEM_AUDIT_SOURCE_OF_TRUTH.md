...
| F-043 | 2026-04-14 | Medium   | Validation consistency                            | **Partially mitigated (2026-04-17):** **`server/src/routes/pricing.ts`** uses shared **`validate`** for **`POST /`** (**`upsertPricing`**), **`POST /tiers`** (**`createTier`**), **`POST /calculate-impact`**, and **`GET /:pricingConfigId/history`** (**UUID** param). Controllers keep business rules (e.g. tier slug regex). **`dashboard`** / **`widget`** remain earlier adopters; broader rollout is still backlog.                                                                                                                                                                                                                                                                                                                          | `server/src/middleware/validateRequest.ts`, `server/src/routes/pricing.ts`, `server/src/routes/dashboard.ts`, `server/src/routes/widget.ts`, `server/src/controllers/businessController.ts`, `server/src/controllers/schedulingController.ts` | Partially mitigated | AI agent |
...
| A-038 | F-043             | Adopt a consistent validation standard for backend routes: apply shared request validation at route boundaries for critical endpoints and reduce controller-local shape parsing to business-rule checks only.                                                                               | Medium   | In progress | **`POST /api/pricing/calculate-impact`** uses **`validate`**; expand to more admin/business routes over time. **`pnpm exec tsc --noEmit`** in **`server`**.                                                                                                                                                                                                                                                                                                               |
