...
| F-043 | 2026-04-14 | Medium   | Validation consistency                            | **Partially mitigated (2026-04-17):** Shared **`validate`** + **`express-validator`** now also guard **`POST /api/pricing/calculate-impact`** (**`tier`**, **`newBasePrice`**, optional **`billingCycle`**). **`dashboard`** / **`widget`** remain the primary prior adopters; broader rollout to other controllers is still backlog.                                                                                                                                                                                                                                                                                                                                                    | `server/src/middleware/validateRequest.ts`, `server/src/routes/pricing.ts`, `server/src/routes/dashboard.ts`, `server/src/routes/widget.ts`, `server/src/controllers/businessController.ts`, `server/src/controllers/schedulingController.ts` | Partially mitigated | AI agent |
