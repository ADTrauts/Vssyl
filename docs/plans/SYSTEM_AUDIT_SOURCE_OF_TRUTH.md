...
Last updated: 2026-04-19 (**Remediation Phases A–F complete** per **D-020**; optional follow-ups **A-051** / **A-052** remain **Deferred**)  
Status: Active  
...
| R-007 | Prisma / schema         | Migration drift, archive confusion, or schema discipline issues may exist.                     | High             | Mitigated (Phase E)  |
...
| R-009 | Shared package hygiene  | Legacy duplicates or parallel exports may create maintenance hazards.                          | Medium           | Mitigated (Phase E)  |
...
| F-043 | 2026-04-14 | Medium   | Validation consistency                            | **Partially mitigated (2026-04-17 / 2026-04-18 / 2026-04-15):** Shared **`validate`** + **`express-validator`** on **`pricing`**, **`contentReports`**, **`notification`**, **`ai-provider-usage`**, **`billing`**, **`member`**, **`business`**, **`scheduling`** (UUID **`id`**; **`POST/PUT`** availability; **`POST /ai/*`**; admin **POST/PUT** schedules/shifts/templates/stations/job locations; swap approve/deny **`managerNotes`**), **`drive`** (**`drive.ts`** + **`file`**/**`folder`**: JWT/UUID, reorder, multipart **`folderId`**/**`dashboardId`**/**`chat`**, **PUT** file/folder/permissions), **`calendar`**, **`chat`**. Remaining: **`admin-portal`** and other large routers as touched; optional **multer** MIME **fileFilter** allowlist; **501** stubs unchanged. | `server/src/middleware/validateRequest.ts`, `server/src/routes/pricing.ts`, `server/src/routes/contentReports.ts`, `server/src/routes/notification.ts`, `server/src/routes/ai-provider-usage.ts`, `server/src/routes/billing.ts`, `server/src/routes/member.ts`, `server/src/routes/scheduling.ts`, `server/src/routes/business.ts`, `server/src/routes/dashboard.ts`, `server/src/routes/widget.ts`, `server/src/routes/drive.ts`, `server/src/routes/calendar.ts`, `server/src/routes/chat.ts`, `server/src/routes/file.ts`, `server/src/routes/folder.ts` | Partially mitigated | AI agent |
| F-044 | 2026-04-14 | High     | Error handling / operational visibility           | **Partially mitigated (2026-04-15 / 2026-04-17):** The Express error middleware in `server/src/index.ts` calls **`logger.error`** (structured) for **all** environments; the 404 catch-all no longer logs every unhandled route in production, and scheduling-availability 404 diagnostics are dev-only. **Production:** responses for **`status >= 500`** use a generic **`message`** while full details stay in logs; **`4xx`** messages still pass through (typical validation/not-found text). Per-request **scheduling** debug middleware and **`PUT /api/profile`** errors use **`logger`** / dev-only **`console`** where applicable. **`server/src/types/tesseract-js.d.ts`** fixes **`tesseract.js`** typing for **`pnpm exec tsc --noEmit`**. **Bootstrap / cron / migrations / fatal exit** in **`index.ts`** use **`logger`** (structured **`operation`** fields). **Remaining:** **`console.*`** when **`logger`** itself fails (registration/login fallbacks) and dev-only scheduling **`console`** dumps.                                                                                                                                                                                                                                                                                   | `server/src/index.ts`, `server/src/lib/logger.ts`, `server/src/types/tesseract-js.d.ts`                                                                                                                                                                                       | Open      | AI agent |
| F-045 | 2026-04-14 | Medium   | Request typing / boundary drift                   | **Mitigated (2026-04-17):** Canonical **`getUserFromRequest`** across controllers and listed routes; **`(req as any).user`** removed under **`server/src`**. **`server/src/express.d.ts`** extends **`Express.Request`** with **`usageInfo`**, **`featureAccess`**, and Multer **`file`** / **`files`**; middleware and controllers use typed **`req`** fields (no **`(req as any)`** for these).                                                                                                                                                                                                                                                                                                                                                    | `server/src/express.d.ts`, `server/src/controllers/`, `server/src/routes/`, `server/src/middleware/`, `server/src/index.ts`                  | Mitigated | AI agent |
...
| F-047 | 2026-04-14 | High     | Frontend architecture / shared shell hotspot      | **Partially mitigated (2026-04-17 / 2026-04-15):** Canonical **`MODULE_ICONS`** in **`web/src/config/moduleIcons.ts`** (includes **`notes`**); **`RightSidebarCustomizer`**, **`DashboardLayout`**, and **`DashboardLayoutWrapper`** consume it (no duplicate sidebar icon maps in those files). **`DashboardLayout`** still centralizes most shell behavior; further splits (hooks / subcomponents) remain.                                                                                                                                                                                                                                                                                                                                               | `web/src/config/moduleIcons.ts`, `web/src/app/dashboard/DashboardLayout.tsx`, `web/src/components/sidebar/RightSidebarCustomizer.tsx`, `web/src/components/business/DashboardLayoutWrapper.tsx`                                                                                                    | Open      | AI agent |
...
| F-056 | 2026-04-14 | High     | Billing / module subscription lifecycle drift     | **Partially mitigated (2026-04-15 / 2026-04-17):** **`StripeService.handleWebhook`** (used by **`POST /api/payment/webhook`**) delegates to **`ModuleSubscriptionService.handleStripeWebhook`** so module rows get **`past_due` / `active` / `cancelled`**. **`ModuleSubscriptionService.createModuleSubscription`** (rollback + real price IDs) as above. **`PaymentService.handleWebhookEvent`** now **delegates to `StripeService`** (no duplicate handler); **`payment_intent.succeeded`** with **`metadata.type === 'module_subscription'`** is handled in **`StripeService`** via **`PaymentService.completeModuleSubscriptionFromPaymentIntent`**. **Tests:** **`server/src/services/__tests__/stripeWebhookBilling.test.ts`**. **Remaining:** `PaymentService` / **`ModuleSubscriptionService`** API alignment; Stripe CLI smoke tests.                                                                                                    | `server/src/services/moduleSubscriptionService.ts`, `server/src/services/paymentService.ts`, `server/src/services/stripeService.ts`                                                                                                     | Open      | AI agent |
...
1. **Wave S — Security / tenant (Critical `A-*`) — closed (2026-04-19)**
  All **Critical** remediation rows in the tracker below are **Done** (former Wave S exemplars, all verified **Done**: ~~`A-001`~~, ~~`A-002`~~, ~~`A-003`~~, ~~`A-009`~~, ~~`A-014`~~, ~~`A-015`~~, ~~`A-023`~~, ~~`A-027`~~, ~~`A-031`~~, ~~`A-032`~~). **No Critical `A-*` remains Backlog.** Use this wave label again only if a new Critical item is filed.
2. **Wave P — Platform entrypoints (Phase 5)** — closed for listed IDs
...
| **E** | Risk register hygiene | **R-007** (Prisma / migration discipline), **R-009** (shared package exports) | **Complete** |
| **F** | Exit: consider the audit initiative complete | **D-020**; criteria satisfied | **Complete** |

**Deferred follow-ups (product roadmap; not blocking audit closure):** **A-051** (module upload / sandbox / env documentation), **A-052** (explicit multimodal downgrade behavior for AI). Tracker status: **Deferred**.

**Remediation execution timeline (historical)**

1. **Phase C:** **Complete** (including **A-036**).
2. **Phase D:** **Complete** (**A-056**).
3. **Parallel:** **A-051** / **A-052** (remain **Deferred**).
4. **Phase E:** **Complete** — **R-007** (`docs/guides/PRISMA_MIGRATION_DISCIPLINE.md`); **R-009** (shared `types` barrel includes search; `docs/guides/SHARED_PACKAGE_IMPORTS.md`). See **D-018**, **D-019**.
5. **Phase F:** **Complete** — no **In progress** rows; only **Deferred** follow-ups (**A-051**, **A-052**); memory bank refreshed (**D-020**).
...
| A-035 | F-040             | Break `schedulingController` into smaller domain-focused controllers/services with explicit ownership boundaries for admin, manager, employee, AI-context, and setup flows. Remove or isolate placeholder endpoints so incomplete surfaces do not live beside production-critical paths.    | High     | Done        | Split into **`server/src/controllers/scheduling/`** (`schedulingShared`, `schedulingDashboardController`, `schedulingAdminController`, `schedulingTeamController`, `schedulingEmployeeController`, `schedulingAiContextController`, `schedulingAdminToolsController`); **`schedulingController.ts`** re-exports for unchanged routes. **`pnpm exec tsc --noEmit`** (server) passed. Unwired analytics stubs remain in admin file — optional follow-up to isolate.                                                                                                                                                           |
| A-036 | F-041             | Split the admin portal into bounded domains and replace `Promise<unknown>` / placeholder-heavy service contracts with typed service interfaces per admin area (users, moderation, billing, support, modules, analytics).                                                                    | High     | Done        | **Routes:** thin **`server/src/routes/admin-portal.ts`** mounts **`registerAdminPortalCoreRoutes`**, **`registerAdminPortalAnalyticsOpsRoutes`**, **`registerAdminPortalPlatformRoutes`** under **`server/src/routes/admin-portal/`**; shared **`requireAdmin`** + content-report allowlist in **`adminPortalShared.ts`**. **Types:** **`server/src/services/admin/adminServiceContracts.ts`**; **`AdminService`** methods that returned **`Promise<unknown>`** now use named contract types / **`AdminJsonObject`**. Dynamic import paths fixed for subdirectory (**`stripeSyncService`**). **`pnpm type-check`** (root) passed. Vitest admin-portal suite may fail in some envs (e.g. **dockerode** resolution) — pre-existing tooling issue.                                                                                                                                                                |
| A-037 | F-042             | Separate module marketplace, developer-submission, artifact-scan, and installation/provisioning responsibilities into clearer services/controllers, then consolidate duplicated submission queries behind one owned path.                                                                   | High     | Done        | Split into **`server/src/controllers/module/`** (`moduleShared`, `moduleProvisionController` install/configure/list-installed + marketplace listing, `moduleBrowseController` categories + details, `moduleSubmissionController`, `moduleRuntimeController`, `moduleArtifactController`); **`moduleController.ts`** re-exports. **`pnpm exec tsc --noEmit`** (server) passed. Optional: dedupe submission queries into a shared service.                                                                                                                                                           |
| A-038 | F-043             | Adopt a consistent validation standard for backend routes: apply shared request validation at route boundaries for critical endpoints and reduce controller-local shape parsing to business-rule checks only.                                                                               | Medium   | Done        | **`scheduling`** admin **PUT**/**PATCH**, swap **PUT** bodies, **`PUT /me/availability/:id`**; **`drive`** multipart fields + **PUT** file/folder/permissions (**`F-043`**). **`pnpm exec tsc --noEmit`** verified in **`server`**. Ongoing: apply same pattern when adding or refactoring routes (**`admin-portal`**, MIME allowlist).                                                                                                                                                                                                                                                                                                                                                  |
| A-039 | F-044             | Normalize backend error handling onto structured logging for both development and production, and stop returning raw internal error text except where explicitly intended and safe.                                                                                                         | High     | Done        | **`buildExpressErrorResponse`** (**`server/src/lib/expressErrorHandlerResponse.ts`**) drives global error JSON (prod **5xx** generic message). Registration/login use **`logWhenLoggerFails`** (**`server/src/lib/safeLoggerFallback.ts`**) instead of raw **`console`** when **`logger`** throws. Tests: **`server/src/lib/__tests__/expressErrorHandlerResponse.test.ts`**.                                                                                                                                                                                                                                                                                                                                                                               |
| A-040 | F-045             | Standardize authenticated request access through shared types/helpers and remove ad hoc `(req as any).user` / duplicate `getUserFromRequest` patterns from controllers as touched.                                                                                                          | Medium   | Done        | **`getUserFromRequest`** in controllers + routes above; **`Express.Request`** augmented in **`server/src/express.d.ts`** (**`usageInfo`**, **`featureAccess`**, Multer **`file`** / **`files`**). **`(req as any).user`** eliminated under **`server/src`**.                                                                                                                                                                                                                                                                                                                                               |
| A-041 | F-046             | Collapse business-configuration ownership to one clear provider strategy per route tree. Remove overlapping `BusinessConfigurationProvider` layers and define how personal dashboard work mode and direct business routes share or hand off business configuration state.                   | High     | Done        | **Dashboard shell:** removed nested `BusinessConfigurationProvider` around **`WorkTab`** in **`DashboardLayoutInner`**; configuration comes from **`DashboardLayout`** (passes `currentBusinessId` when set). **`BrandedWorkDashboard`** uses that context. **`/business/[id]`** layouts unchanged (own route tree). Manual QA: Work tab + branded dashboard after auth.                                                                                                                                          |
| A-042 | F-047             | Split `DashboardLayout` into smaller shell components/hooks and move shared registries/constants (such as module icon mappings) out of route modules into stable shared frontend modules.                                                                                                   | High     | Done        | **`MODULE_ICONS`** in **`web/src/config/moduleIcons.ts`**. Shell split: **`web/src/app/dashboard/DashboardLayout.tsx`** (providers + **`DashboardLayoutWithModules`**) vs **`web/src/app/dashboard/DashboardLayoutInner.tsx`** (main UI). Further hook extraction optional. Manual QA dashboard/sidebar when convenient.                                                                                                                                                                                                                                                                                                 |
| A-043 | F-048             | Standardize business workspace navigation on one primary routing model, then align embedded module shells, nested routes, and active-module detection to that model. Prioritize HR as a representative cleanup path.                                                                        | Medium   | Done        | **`web/src/lib/businessWorkspaceNavigation.ts`**: **`resolveBusinessWorkspaceModule`**, **`hasNestedWorkspaceRoute`**, **`buildBusinessWorkspaceModuleHref`**. **`DashboardLayoutWrapper`** uses **`useSearchParams`** (no `window`) + shared helpers; hub **`workspace/page.tsx`** uses same resolver. Optional follow-up: default all module links to segment URLs (`/workspace/hr`) and add automated tests when **`web`** test runner exists.                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| A-050 | F-056             | Consolidate module billing and Stripe lifecycle handling behind one authoritative service path, use real Stripe product/price mapping, and ensure webhook processing updates both platform and module subscription state consistently.                                                      | High     | Done        | Single webhook path via **`StripeService.handleWebhookEvent`**. Automated coverage: **`stripeWebhookBilling.test.ts`** (delegation + module rows) + **`stripeService.paymentIntentModuleSubscription.test.ts`** (**`payment_intent.succeeded`** → **`PaymentService.completeModuleSubscriptionFromPaymentIntent`**). Optional per environment: Stripe CLI smoke test; E2E.                                                                                                                                                                                                                                                                                                                  |
| A-051 | F-057             | Align module upload, sandbox, and runtime validation expectations with actual deployment environments. Document required infrastructure explicitly and avoid implying sandbox guarantees that depend on unavailable local Docker/runtime assumptions.                                       | Medium   | Deferred    | **Doc slice (2026-04-19):** `docs/guides/MODULE_PLATFORM_ENVIRONMENT_MATRIX.md` + link from `docs/deployment/MODULE_UPLOAD_PHASE7_ROLLOUT_GUIDE.md`. Remaining optional: focused backend tests / production-safety review.                                                                                                                                                                                                                                                                                                               |
| A-052 | F-058             | Make multimodal capability downgrade explicit in platform behavior: surface when attachments are reduced to summaries/text-only, and enforce or document the supported provider/model combinations for image-aware requests.                                                                | Medium   | Deferred    | Explicitly deferred; not required for audit exit (**D-020**). Focused backend tests, manual AI attachment validation when picked up.                                                                                                                                                                                                                                                                                                                                         |
...
| D-010 | 2026-04-19 | **Phase B (Wave S / execution-queue reconciliation)** is documentation-only: align the remediation execution queue with the tracker so “examples” are not mistaken for open Critical work. | The tracker already showed Wave S exemplars as **Done**; the queue text lagged and implied outstanding Critical backlog. | Wave S and Wave P/R lists are marked closed for current IDs; **R-007** / **R-009** mitigated in **Phase E** (**D-018**, **D-019**); lettered remediation closed in **Phase F** (**D-020**), with **A-051**/**A-052** left **Deferred**. |
| D-011 | 2026-04-19 | **`schedulingController` split** keeps a single barrel (`schedulingController.ts`) so **`routes/scheduling.ts`** and tests need no import-path churn; submodules live under **`controllers/scheduling/`**. | Matches **A-035** goal (clearer boundaries) without risking route/controller name drift. | Dynamic imports to **`hrScheduleService`** use **`../../services/`** from the subdirectory. |
...
| D-015 | 2026-04-19 | **Lettered phases A–F** name the *remediation execution plan* (close targeted rows → reconcile queue → architecture cluster → tests/rails → risk hygiene → exit). They are **not** the same labels as audit **Phases 1–6** (read-only audit passes, all complete). | Avoids confusion between “Phase C” (remediation) and “Phase 4” (frontend audit pass). | **Phases A–F** complete (**D-020**). Optional follow-ups: **A-051**/**A-052** (**Deferred**). |
...
| D-018 | 2026-04-19 | **R-007:** Publish **`docs/guides/PRISMA_MIGRATION_DISCIPLINE.md`** as the operational runbook; it references existing modular-schema + migration workflow in coding standards and CI **`migrate deploy`**. | Closes the “discipline may exist only in rules” gap with a short human-facing guide. | Ongoing schema work still follows **`pnpm prisma:build`** and migrations; drift handling remains per coding standards. |
| D-019 | 2026-04-19 | **R-009:** **`shared/src/types/index.ts`** re-exports **`./search`**; **`shared/src/index.ts`** uses **`export * from './types'`** (single types surface). Add **`docs/guides/SHARED_PACKAGE_IMPORTS.md`** describing canonical **`shared/types`** / **`shared/components`** paths and avoiding deep **`shared/src/...`** imports in app code. | Reduces parallel export paths and documents the preferred import style for future refactors. | Deep imports may remain until opportunistically normalized; new code should follow the guide. |
| D-020 | 2026-04-19 | **Phase F (audit remediation exit)** is satisfied: the **Remediation Tracker** has no **In progress** rows; remaining scope is **Deferred** (**A-051**, **A-052**) with explicit rationale; **`memory-bank/activeContext.md`** and **`memory-bank/progress.md`** note closure so ongoing work is not mistaken for open audit backlog. | Closes lettered phases **A–F** without forcing optional medium items into **Done**. | Product work continues per roadmap; audit source of truth remains **`docs/plans/SYSTEM_AUDIT_SOURCE_OF_TRUTH.md`**. |
...
2. `F-044` (partially mitigated: structured logging + prod **5xx** generic **`message`**; remaining **`console.*`** audit)
  Finish production-safe error handling and structured logging before deeper backend refactors, so later changes are easier to observe and verify.
3. `F-043`, `F-045`
...
2. ~~`F-055`~~, `F-056` (partially mitigated — single webhook + price IDs + rollback + **`PaymentService`** delegates to **`StripeService`**; add tests)
...
2. **Remediation Phases A–F:** complete (**D-020**).
3. **Deferred follow-ups:** **A-051** / **A-052** — see **Remediation Tracker** and **D-020**.

### Phase B — Wave S / queue reconciliation (2026-04-19)

**Status:** Complete.

**What changed:** Execution-queue bullets (**Wave S**, **Wave P**, **Wave R**) were reconciled with the `A-*` table: Critical security/tenant items are **Done**; queue text no longer implies open Critical backlog. See **D-010**.
