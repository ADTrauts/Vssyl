...
| F-043 | 2026-04-14 | Medium   | Validation consistency                            | **Partially mitigated (2026-04-17 / 2026-04-18):** Shared **`validate`** + **`express-validator`** on **`pricing`**, **`contentReports`**, **`notification`**, **`ai-provider-usage`**, **`billing`**, **`member`**, plus **`scheduling`** (UUID **`id`** params on entity routes; **`POST /me/availability`** body/query for availability; removed per-request debug **`console.log`** middleware from **`scheduling`** router) and **`business`** (create/update/invite/member/logo path UUIDs and bodies). Remaining: **`drive`**, **`calendar`**, **`chat`**, deeper **`scheduling`** POST bodies (**`/ai/*`**, admin creates).                                                                                                                                                                                                                    | `server/src/middleware/validateRequest.ts`, `server/src/routes/pricing.ts`, `server/src/routes/contentReports.ts`, `server/src/routes/notification.ts`, `server/src/routes/ai-provider-usage.ts`, `server/src/routes/billing.ts`, `server/src/routes/member.ts`, `server/src/routes/scheduling.ts`, `server/src/routes/business.ts`, `server/src/routes/dashboard.ts`, `server/src/routes/widget.ts` | Partially mitigated | AI agent |
...
| A-038 | F-043             | Adopt a consistent validation standard for backend routes: apply shared request validation at route boundaries for critical endpoints and reduce controller-local shape parsing to business-rule checks only.                                                                               | Medium   | In progress | **`scheduling`** + **`business`** added per **`F-043`**. Next: **`drive`**, **`calendar`**, **`chat`**, **`scheduling`** AI/admin POST payloads. **`pnpm exec tsc --noEmit`** in **`server`**.                                                                                                                                                                                                                                                                                                                                                  |
