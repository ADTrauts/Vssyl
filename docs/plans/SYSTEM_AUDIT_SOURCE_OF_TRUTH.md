...
| F-045 | 2026-04-14 | Medium   | Request typing / boundary drift                   | **Partially mitigated (2026-04-17):** Canonical **`getUserFromRequest`** in controllers (prior pass) plus **`routes/`** (**`moduleAIContext`**, **`contentReports`**, **`pricing`**, **`ai-provider-usage`**, admin HR/subscription/seed routes), **`subscriptionMiddleware`**, and **`index`** API request logging. **`(req as any).user`** removed under **`server/src`**. **Remaining:** **`(req as any).file`** (multer), **`(req as any).usageInfo` / `.featureAccess`** (middleware attachments), or extend **`Express.Request`** typing.                                                                                                                                                                                                                                                                                                                       | `server/src/controllers/`, `server/src/routes/`, `server/src/middleware/`, `server/src/index.ts`                                          | Open      | AI agent |
...
| A-040 | F-045             | Standardize authenticated request access through shared types/helpers and remove ad hoc `(req as any).user` / duplicate `getUserFromRequest` patterns from controllers as touched.                                                                                                          | Medium   | In progress | Controllers + **`routes/`** (**`moduleAIContext`**, **`contentReports`**, **`pricing`**, **`ai-provider-usage`**, admin emergency routes) + **`subscriptionMiddleware`** + **`index`** request logger use **`getUserFromRequest`**. **`(req as any).user`** eliminated under **`server/src`**. Optional: typed **`multer`** **`file`**; mark **A-040** Done after review.                                                                                                                                                                                                                                                                                                                                                                                                                                |
