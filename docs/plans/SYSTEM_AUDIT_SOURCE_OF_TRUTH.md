...
| F-043 | 2026-04-14 | Medium   | Validation consistency                            | **Partially mitigated (2026-04-17 / 2026-04-18):** Shared **`validate`** + **`express-validator`** on **`pricing`**, **`contentReports`**, **`notification`**, **`ai-provider-usage`**, plus **`billing`** (subscription/checkout/module subscription/usage/invoice/payment-method bodies and UUID params) and **`member`** (UUID path params for business/member/invitation/pinned routes, **`GET /users/search`** query bounds, **`POST .../pinned`** body). Controllers still enforce auth and business rules; many **`member`** POST bodies remain **Zod**-parsed in **`memberController`**. Remaining: **`scheduling`**, **`business`**, **`drive`**, and broader sweeps.                                                                                                                                                                                                                    | `server/src/middleware/validateRequest.ts`, `server/src/routes/pricing.ts`, `server/src/routes/contentReports.ts`, `server/src/routes/notification.ts`, `server/src/routes/ai-provider-usage.ts`, `server/src/routes/billing.ts`, `server/src/routes/member.ts`, `server/src/routes/dashboard.ts`, `server/src/routes/widget.ts` | Partially mitigated | AI agent |
...
| A-038 | F-043             | Adopt a consistent validation standard for backend routes: apply shared request validation at route boundaries for critical endpoints and reduce controller-local shape parsing to business-rule checks only.                                                                               | Medium   | In progress | **`billing`** + **`member`** route-level validation landed; see **`F-043`**. Next: **`scheduling`**, **`business`**, **`drive`**, **`calendar`**. **`pnpm exec tsc --noEmit`** in **`server`**.                                                                                                                                                                                                                                                                                                                                                |
