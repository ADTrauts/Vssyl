...
| F-043 | 2026-04-14 | Medium   | Validation consistency                            | **Partially mitigated (2026-04-17 / 2026-04-18):** Shared **`validate`** + **`express-validator`** on **`pricing`** (admin POSTs + history UUID), **`contentReports`** (**`POST /`**, **`GET /my-reports`** query pagination), **`notification`** (**`POST /`**, **`POST /:id/read`** UUID), **`ai-provider-usage`** (optional ISO8601 **`startDate`/`endDate`**, **`provider`/`groupBy`/`period`** where applicable). **`dashboard`** / **`widget`** unchanged. Remaining: **`billing`**, **`member`**, **`scheduling`**, and other high-traffic routers.                                                                                                                                                                                                                                                         | `server/src/middleware/validateRequest.ts`, `server/src/routes/pricing.ts`, `server/src/routes/contentReports.ts`, `server/src/routes/notification.ts`, `server/src/routes/ai-provider-usage.ts`, `server/src/routes/dashboard.ts`, `server/src/routes/widget.ts` | Partially mitigated | AI agent |
...
| A-038 | F-043             | Adopt a consistent validation standard for backend routes: apply shared request validation at route boundaries for critical endpoints and reduce controller-local shape parsing to business-rule checks only.                                                                               | Medium   | In progress | **`pricing`**, **`contentReports`** (**`contentReportController`**), **`notification`**, **`ai-provider-usage`** per **`F-043`**. Next: **`billing`**, **`member`**, **`business`**, **`scheduling`**. **`pnpm exec tsc --noEmit`** in **`server`**.                                                                                                                                                                                                                                                                                         |
