...
| F-043 | 2026-04-14 | Medium   | Validation consistency                            | **Partially mitigated (2026-04-17 / 2026-04-18 / 2026-04-15):** Shared **`validate`** + **`express-validator`** on **`pricing`**, **`contentReports`**, **`notification`**, **`ai-provider-usage`**, **`billing`**, **`member`**, plus **`scheduling`** (UUID **`id`** params; **`POST /me/availability`**; **`POST /ai/*`** bodies; admin **POST** creates: schedules, shifts, schedule templates, stations, job locations) and **`business`**. **`drive`**: JWT + UUID on **`drive.ts`** routes; nested **`file`** / **`folder`** routers: UUID **`id`** / permission **`userId`**; reorder path **`null`** or UUID; reorder array bodies; create/move/permission bodies. **`calendar`**, **`chat`** as prior. Remaining: **`scheduling`** admin **PUT**/**PATCH** payloads and future non-501 routes; **`drive`** multipart **upload** field-level validation if needed. | `server/src/middleware/validateRequest.ts`, `server/src/routes/pricing.ts`, `server/src/routes/contentReports.ts`, `server/src/routes/notification.ts`, `server/src/routes/ai-provider-usage.ts`, `server/src/routes/billing.ts`, `server/src/routes/member.ts`, `server/src/routes/scheduling.ts`, `server/src/routes/business.ts`, `server/src/routes/dashboard.ts`, `server/src/routes/widget.ts`, `server/src/routes/drive.ts`, `server/src/routes/calendar.ts`, `server/src/routes/chat.ts`, `server/src/routes/file.ts`, `server/src/routes/folder.ts` | Partially mitigated | AI agent |
...
| A-038 | F-043             | Adopt a consistent validation standard for backend routes: apply shared request validation at route boundaries for critical endpoints and reduce controller-local shape parsing to business-rule checks only.                                                                               | Medium   | In progress | **`scheduling`** AI + admin **POST** bodies; **`drive`** **`file`** / **`folder`** routers (**`F-043`**). Next: **`scheduling`** admin **PUT**/**PATCH**; **`drive`** upload. **`pnpm exec tsc --noEmit`** verified in **`server`**.                                                                                                                                                                                                                                                                                                                                                  |
