...
Historical record extracted from `memory-bank/systemPatterns.md` on 2026-09-03 (Batch 1B-2).

**Not** current architecture authority. Canonical architecture lives under `docs/architecture/`. Current pattern index: `memory-bank/systemPatterns.md`.
...
- **2025-01: Added Chat Dashboard Tabs Pattern (dashboard-based tab switching in floating chat, business dashboard inclusion, core module treatment, unread counts per dashboard).**
...
- **2025-02: Added AI Vision & Attachment Architecture patterns (attachment flow, GCS/storageService for Cloud Run, vision parts and provider capabilities, docs/ai/ and golden rules). Vision optional items implemented: image resize (resizeImageForVision, 1600px JPEG), CSV table parsing (parseCsvToMarkdownTable), DALL·E image generation (POST /api/ai/generate-image), usedVisionParts badge in ai-chat and AIChatDropdown.**
...
- See [chatProductContext.md](./chatProductContext.md), [driveProductContext.md](./driveProductContext.md), [dashboardProductContext.md](./dashboardProductContext.md), [todoProductContext.md](./todoProductContext.md), and [marketplaceProductContext.md](./marketplaceProductContext.md) for module-specific architecture and patterns.
...
- **AI vision & attachments**: See **docs/ai/ARCHITECTURE.md** (attachment flow, context.fileIds → getFileSummaries + getVisionImageParts → prompt + visionImageParts → provider), **docs/ai/PROVIDERS.md** (request shapes, capability matrix, model selection), **docs/ai/RUNBOOK.md** (logging, Cloud Run, env vars), **docs/ai/GOLDEN_RULES.md** (Cloud Run cannot rely on local uploads; GCS fetch; vision requires parts + provider support + model + multimodal request; always log files.length, visionParts.length, model, request shape). When attachments exist, ensure getVisionImageParts is called and visionImageParts passed to the provider. Do not assume /uploads in Cloud Run — use GCS and storageService. When adding a new provider, declare vision capabilities and use vision model when images exist.
...
│       └─ [Dashboard override support for tab-based context switching]
└─ GlobalTrashProvider
...
- **ChatProvider**: Unified chat state shared between global floating chat and main chat panels
  - **Dashboard Override**: `setDashboardOverride(dashboardId)` and `clearDashboardOverride()` allow floating chat to switch contexts independently of current dashboard
  - **Effective Dashboard ID**: Uses `dashboardOverride ?? currentDashboardId` to determine which conversations to load
  - **Core Module**: Chat is always available on all dashboards - no widget check required
  - **Dashboard Tabs**: Floating chat (StackableChatContainer, UnifiedGlobalChat) includes dashboard tabs for quick context switching
...
- **Listeners**:
  - File Hub: `web/src/components/modules/DriveModule.tsx` → reloads `loadFilesAndFolders()`
...
All modules scoped by businessDashboardId + businessId
    ├─ File Hub: files filtered by dashboardId + businessId
    ├─ Chat: conversations filtered by dashboardId + businessId
    │   └─ Dashboard tabs in floating chat allow quick context switching between personal and business dashboards
...
**Examples:**
- `/app/drive/` - File Hub module with recent, shared, starred, trash sub-pages
...
#### **Marketplace Module Pattern (To-Do Example)**

Marketplace modules are installable modules (not core modules) that users install from the marketplace.

**Key Characteristics:**
- Status: `APPROVED` (visible in marketplace)
- Auto-seeded on server startup
- Supports personal, business, and household contexts
- Requires AI context integration (mandatory)
- Installation tracked via `ModuleInstallation` or `BusinessModuleInstallation`

**Example Structure:**
- `/app/todo/` - Personal context
- `/app/business/[id]/workspace/todo/` - Business context
- Database scoping: `dashboardId` + `businessId` (null for personal)

**See**: [todoProductContext.md](./todoProductContext.md) for complete implementation details
...
**File Hub Module Variants:**
- `DriveModule.tsx` - Standard File Hub for personal/basic business
- `EnhancedDriveModule.tsx` - Enterprise File Hub with bulk operations
...
// BEFORE (BROKEN)
DATABASE_URL=postgresql://[REDACTED — historical credential removed during 2026-09 security remediation]
...
## [2025-01] To-Do Module Architecture Pattern ✅

### **Marketplace Module Implementation Pattern**

The To-Do module demonstrates the complete pattern for marketplace-installable modules (non-core modules that users install from the marketplace).

#### **Module Registration Pattern**

```typescript
// server/src/startup/seedTodoModule.ts
// Auto-seeds module on server startup (non-blocking)
export async function seedTodoModuleOnStartup(): Promise<void> {
  // 1. Check if module exists
  const existing = await prisma.module.findUnique({ where: { id: 'todo' } });
  if (existing) return;
  
  // 2. Get system user (admin or first user)
  const systemUser = await prisma.user.findFirst({ where: { role: 'ADMIN' } });
  
  // 3. Create Module record with:
  await prisma.module.create({
    data: {
      id: 'todo',
      name: 'To-Do',
      status: 'APPROVED',  // Visible in marketplace
      pricingTier: 'free',  // No subscription required
...
#### **Multi-Context Task Scoping Pattern**

**Critical Rule**: All queries MUST include `dashboardId` for multi-tenant isolation.

```typescript
// Backend: Always scope by dashboardId + businessId
const where: Prisma.TaskWhereInput = {
  dashboardId: dashboardId,  // Required - prevents data leakage
  businessId: businessId || null,  // Null for personal, set for business
  trashedAt: null,
};

// Frontend: Use DashboardContext
...
#### **AI Context Integration Pattern (Mandatory)**

Every marketplace module MUST implement AI context integration:

```typescript
// 1. Register in registerBuiltInModules.ts
{
  moduleId: 'todo',
  moduleName: 'To-Do',
  aiContext: {
    purpose: 'Task and to-do management with AI-powered prioritization',
    category: 'PRODUCTIVITY',
    keywords: ['task', 'todo', 'deadline', ...],
    patterns: ['show my tasks', 'what\'s due today', ...],
    contextProviders: [
      { name: 'overview', endpoint: '/api/todo/ai/context/overview' },
      { name: 'upcoming', endpoint: '/api/todo/ai/context/upcoming' },
      // ... more providers
    ]
  }
}

// 2. Implement context controller
// server/src/controllers/todoAIContextController.ts
export async function getOverviewContext(req: Request, res: Response) {
  // Standardized response format:
  res.json({
    success: true,
    context: { summary: {...}, details: {...} },
    metadata: { provider: 'todo', endpoint: 'overview', ... }
  });
}
```
...
```
User visits /modules marketplace
  ↓
Sees "To-Do" module (status: APPROVED, pricing: free)
  ↓
Clicks "Install" button
  ↓
POST /api/modules/todo/install?scope=personal
  ↓
Creates ModuleInstallation record
  ↓
Module appears in user's installed modules list
  ↓
User can add module widget to dashboard
  ↓
Access at /todo (personal) or /business/[id]/workspace/todo (business)
```

#### **Files Structure Pattern**

```
prisma/modules/[module]/
  └─ [module].prisma              # Database schema

server/src/
  ├─ controllers/
  │   ├─ [module]Controller.ts     # Main CRUD operations
  │   └─ [module]AIContextController.ts  # AI context providers
  ├─ routes/
  │   └─ [module].ts               # API route definitions
  └─ startup/
      └─ seed[Module]Module.ts     # Marketplace registration

web/src/
  ├─ app/[module]/
  │   └─ page.tsx                  # Main page
  ├─ components/[module]/
  │   ├─ [Module]Module.tsx        # Main component
  │   └─ [subcomponents].tsx       # Feature components
  └─ api/
      └─ [module].ts               # Type-safe API client
```

#### **Key Principles**

1. **Marketplace Registration**: Module seeded with status 'APPROVED' on server startup
2. **Multi-Context Support**: Manifest declares supported contexts (personal/business/household)
3. **Dashboard Scoping**: All queries MUST include dashboardId for isolation
4. **AI Integration**: Mandatory AI context registration with 2-4 context providers
5. **Free Tier Default**: Set pricingTier to 'free' unless premium features required
6. **Installation Tracking**: Use ModuleInstallation or BusinessModuleInstallation records

**See**: [todoProductContext.md](./todoProductContext.md) for complete To-Do module documentation

---

## [2025-01] To-Do Module Calendar Integration Pattern ✅

The To-Do module demonstrates automatic calendar event creation and synchronization patterns for cross-module integration.

### **Automatic Calendar Event Creation Pattern**

Tasks with due dates automatically create calendar events in the user's personal primary calendar:

```typescript
// server/src/controllers/todoController.ts

// Helper function called automatically on task create/update
async function ensureTaskCalendarEvent(taskId: string, userId: string): Promise<void> {
  ...
  // Get or auto-provision personal primary calendar
  ...
  const endAt = task.timeEstimate 
    ? new Date(startAt.getTime() + task.timeEstimate * 60000)
    : new Date(startAt.getTime() + 3600000); // 1 hour default
...
// Integrated into createTask and updateTask
export async function createTask(req: Request, res: Response) {
  const task = await prisma.task.create({ ... });
  
  // Automatically create calendar event if due date provided
  if (task.dueDate) {
...
### **Calendar Context Filtering Pattern**

Calendar pages must pass dashboard IDs (not formatted context strings) for proper context resolution:

```typescript
// ❌ WRONG: Passing formatted context string
const contextFilter = [`PERSONAL:${dashboardId}`];

// ✅ CORRECT: Pass dashboard ID directly
const contextFilter = useMemo(() => {
  if (!currentDashboard) return [];
  const type = getDashboardType(currentDashboard).toUpperCase();
  
  if (type === 'PERSONAL') {
    // Backend resolves dashboard ID to PERSONAL:userId
    return [currentDashboard.id];
  } else {
    // For business/household, pass businessId/householdId
    const id = currentDashboard.business?.id || currentDashboard.household?.id;
    return id ? [id] : [currentDashboard.id];
  }
}, [currentDashboard, getDashboardType]);

// Backend automatically resolves:
// - Personal dashboard ID → PERSONAL:userId
// - Business dashboard ID → BUSINESS:businessId
// - Household dashboard ID → HOUSEHOLD:householdId
```

### **Calendar View Integration Pattern**

Calendar view components display both tasks and calendar events:

```typescript
// web/src/components/todo/TaskCalendar.tsx

// Load both tasks and calendar events
const loadCalendarEvents = async () => {
  ...
  if (primaryCalendar) {
    const events = await calendarAPI.listEvents({
      ...
      contexts: [dashboardId] // Pass dashboard ID, not formatted string
    });
    setCalendarEvents(events.data);
  }
};

// Display both in calendar view
...
1. **Automatic Event Creation**: Tasks with due dates automatically create calendar events (no manual action required)
2. **Event Synchronization**: Task updates (title, description, due date) automatically sync to linked calendar events
3. **No Duplicates**: System checks for existing linked events before creating new ones
4. **Personal Calendar Default**: Events created in user's personal primary calendar (auto-provisioned if missing)
5. **Context Resolution**: Backend resolves dashboard IDs to proper context format automatically
6. **Error Handling**: Calendar event creation failures don't fail task creation/update (logged but non-blocking)

**See**: [todoProductContext.md](./todoProductContext.md) for complete calendar integration documentation

---

## [2025-01-22] Backend Performance Optimization Patterns ✅

### **Database Connection Pool Scaling**

Connection pool size should scale with environment and load:

```typescript
// server/src/lib/prisma.ts
const connectionLimit = process.env.NODE_ENV === 'production' ? 20 : 5;
if (!dbUrl.includes('connection_limit=')) {
  dbUrl += `${separator}connection_limit=${connectionLimit}&pool_timeout=20&connect_timeout=60`;
}
```

**Pattern**:
- **Production**: 20 connections (handles concurrent load)
- **Development**: 5 connections (saves resources)
- **Connection Parameters**: `pool_timeout=20&connect_timeout=60` for proper timeout handling

**Why**: Small connection pools (5) cause bottlenecks under load, leading to 504 errors and WebSocket connection failures.
...
model AdminImpersonation {
  ...
  @@index([adminId])
  @@index([adminId, endedAt]) // Composite index for common query pattern
  @@map("admin_impersonations")
}
```

**Pattern**:
- Single-column indexes: `@@index([adminId])`
- Composite indexes: `@@index([adminId, endedAt])` for queries filtering by multiple columns
- **Query Pattern**: `WHERE adminId = ? AND endedAt IS NULL` → uses composite index

**Why**: Composite indexes dramatically speed up multi-column WHERE clauses, reducing query time from seconds to milliseconds.

### **Request Timeout Middleware**

Always add request timeout middleware to prevent hanging requests:

```typescript
// server/src/index.ts
app.use((req: Request, res: Response, next: NextFunction) => {
  const timeoutDuration = req.path.includes('/upload') || req.path.includes('/file') 
    ? 120000  // 2 minutes for file uploads
    : 30000;  // 30 seconds for most requests
...
- **File Uploads**: 2 minutes timeout
- **Response Check**: Only send timeout if response not already sent

**Why**: Prevents requests from hanging indefinitely, providing clear 504 errors instead of silent failures.

### **Query Timeout Protection**

Protect slow database queries with timeout wrappers:
...
```typescript
// server/src/lib/queryTimeout.ts
...
const result = await withQueryTimeout(
  ...
  10000,
  'Impersonation query timeout'
);
```

**Pattern**:
- Use `Promise.race()` to add timeout to any query
- Default timeout: 10 seconds for most queries
- Custom error messages for better debugging
- Reusable utility function for consistency

**Why**: Prevents database queries from blocking requests indefinitely, especially important for endpoints that experience high load.

### **Performance Optimization Checklist**

When experiencing 504 errors or backend overload:

1. ✅ **Check Connection Pool Size**: Increase if experiencing connection exhaustion
2. ✅ **Add Composite Indexes**: For queries filtering by multiple columns
3. ✅ **Add Request Timeouts**: Prevent hanging requests
4. ✅ **Add Query Timeouts**: Protect slow database queries
5. ✅ **Monitor Error Rates**: Track 504 errors and WebSocket failures
6. ✅ **Profile Slow Queries**: Use Prisma query logging to identify bottlenecks

### **Key Principles**

1. **Connection Pool Scaling**: Production needs more connections than development
2. **Index Optimization**: Composite indexes for multi-column WHERE clauses
3. **Timeout Protection**: Always add timeouts to prevent hanging
4. **Error Clarity**: Provide clear timeout errors instead of silent failures
5. **Reusable Patterns**: Create utilities for common timeout patterns

**See**: [troubleshooting.md](./troubleshooting.md) for complete performance troubleshooting guide
