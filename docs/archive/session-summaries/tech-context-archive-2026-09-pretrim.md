...
Historical record extracted from `memory-bank/techContext.md` on 2026-09-03 (Batch 1B-2).

**Not** current stack authority. Prefer package.json / `docs/deployment/` / current `memory-bank/techContext.md`.
...
CREATE USER vssyl_user WITH PASSWORD '[REDACTED — historical credential removed during 2026-09 security remediation]';
...
**Local test accounts (development)**:
- **Primary app tester**: `place.tester@vssyl.local` / `password123` — **platform `ADMIN`** (can use `/admin-portal` for module review, etc.). Created by `server/src/scripts/seedPlaceDemoData.ts` and ensured by `server/scripts/seed-test-data.ts` (includes membership on the seeded **Test Business** for business-scoped flows). Re-running either seed promotes this user to `ADMIN` if they already existed as `USER`.
- **Legacy seeds**: `test@example.com` / `test123`, `business@example.com` / `business123`, `admin@blockonblock.com` / `admin123` (admin portal).
- Reset the Place tester password: run `server/scripts/reset-test-user-password.ts` (resets `place.tester@vssyl.local` to `password123`).
