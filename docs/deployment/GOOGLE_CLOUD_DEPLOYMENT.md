...
### Re-authenticate gcloud and inspect failed Cloud Build

If you see **"Reauthentication failed"** or the build failed and you need to inspect it:

1. **Log in to Google Cloud (opens browser).** Run this in your terminal (use the gcloud from the project’s Google CLI folder if that’s what you use):

   ```bash
   # From project root; use the SDK in "Google CLI" if you use that
   "./Google CLI/google-cloud-sdk/bin/gcloud" auth login
   ```

   Or if `gcloud` is on your PATH:

   ```bash
   gcloud auth login
   ```

2. **Set project and list recent builds:**

   ```bash
   gcloud config set project vssyl-472202
   gcloud builds list --limit=10 --format="table(id,status,createTime,source.repoSource.branchName)"
   ```

3. **Stream logs for a specific build (replace BUILD_ID with the id from the list):**

   ```bash
   gcloud builds log BUILD_ID --stream
   ```

   Or open the build in the console to see step-by-step logs:

   ```bash
   open "https://console.cloud.google.com/cloud-build/builds?project=vssyl-472202"
   ```

4. **Optional: use application default credentials for local scripts:**
...
- [Vssyl memory bank (architecture & product context)](../../memory-bank/)
