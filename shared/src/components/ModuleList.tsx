...
export const ModuleList: React.FC<ModuleListProps> = ({ modules, onSelect, renderActions }) => (
  <div className="flex flex-col gap-4">
    {modules.map((mod) => (
      <div key={mod.id} className="flex items-center bg-white dark:bg-slate-900 rounded shadow p-4 gap-4">
        <div className="text-3xl">{mod.icon || '🧩'}</div>
        <div className="flex-1">
          <div className="font-bold text-lg">{mod.name}</div>
          <div className="text-gray-600 dark:text-gray-400 text-sm">{mod.description}</div>
        </div>
