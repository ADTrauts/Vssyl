...
export async function sendVerificationEmail(email: string, token: string): Promise<SendEmailResult> {
  const verificationUrl = `${getAppBaseUrl()}/auth/verify-email?token=${token}`;
...
export async function sendPriceChangeNotification(params: {
  ...
  const { billing, replyTo } = getEmailAddressDefaults();
      ...
      <body>
        <div class="container">
          ...
          ${
            isIncrease
              ? `
            <p>This change will take effect on your next billing cycle. If you have any questions or concerns, please contact us at ${replyTo}.</p>
          `
              : `
          ...
          <div class="footer">
            <p>Questions about billing? Contact ${billing}.</p>
...
export async function sendBusinessInvitationEmail(
  ...
  const invitationUrl = `${getAppBaseUrl()}/auth/accept-invitation?token=${token}`;
