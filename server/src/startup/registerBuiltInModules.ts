...
const BUILT_IN_MODULES: Array<{ moduleId: string; moduleName: string; aiContext: ModuleAIContext }> = [
  {
    ...
    aiContext: {
      ...
      patterns: [
        ...
        '(files?|documents?) shared with me',
        '(file|document)s? .+ shared with me',
        '(files?|documents?) (owned by|from) .+',
        'who owns .+',
        'accessible (files?|documents?) from .+',
        '(files?|documents?) .+ I can access',
      ],
  ...
  },
  {
    ...
    aiContext: {
      ...
      keywords: [
        ...
        'hr overview', 'pto policy', 'our pto policy', 'employee count',
        'who manages', 'manager of', "who's off", 'on leave',
      ],
      patterns: [
        'hr (system|module|dashboard|overview)',
        '(?:give me|show me|get).{0,24}hr overview',
        '(?:our|the) hr (overview|status)',
        'employee (list|directory|database)',
        'how many employees',
        'employee count',
        'how many people work',
        'who(?:\'s| is) (?:off|out|on leave|on pto|has pto)',
        'who manages',
        'manager of (?:the )?[\\w\\s\'-]+',
        'our pto policy',
      ...
      contextProviders: [
        ...
        },
        {
          ...
          description: 'Get the authenticated user\'s position, department, and manager in this business',
          endpoint: '/api/hr/ai/context/self-employment',
          cacheDuration: 300000,
          supportedIntents: ['business_operations', 'general_chat'],
          retrievalCost: 'low',
          priority: 80,
          pipelineSourceIds: ['module_context'],
          volatility: 'slow',
