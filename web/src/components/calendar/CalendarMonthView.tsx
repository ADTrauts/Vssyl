...
import { CalendarShortcutsHelp } from './CalendarShortcutsHelp';
...
export function CalendarMonthView({
  ...
  const [pendingRecurrenceMove, setPendingRecurrenceMove] = useState<{
    ev: EventItem;
  ...
  const openEditDrawer = useCallback((event: EventItem) => {
  ...
  const handleExportMonth = useCallback(async () => {
    try {
      ...
      toast.error('Error exporting calendar. Please try again.');
    }
  }, [viewDate, visibleCalendarIds, contextFilter]);
  ...
  const viewSwitcher = (
    <div className="flex flex-wrap items-center gap-2">
      <div className="flex items-center gap-1 rounded-lg bg-gray-100 p-1 dark:bg-gray-700">
        <a
          className="rounded-md px-2 py-1 text-xs font-medium text-gray-700 transition-all hover:bg-gray-200 dark:text-gray-300 dark:hover:bg-gray-600"
          href="/calendar/day"
        >
          Day
        </a>
        ...
        <a
          className="rounded-md px-2 py-1 text-xs font-medium text-white shadow-sm"
          href="/calendar/month"
      ...
      <div className="flex items-center gap-1">
        <button
          type="button"
          className="rounded-lg bg-gray-100 p-1.5 transition-colors hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600"
        ...
        <span className="rounded-lg bg-gray-100 px-3 py-1.5 text-sm font-semibold text-gray-900 dark:bg-gray-700 dark:text-gray-100">
          {monthLabel}
        </span>
      </div>
    </div>
  ...
  return (
    <>
    <CalendarPageShell
      sidebarContextType={contextTypeProp ?? (businessId ? 'BUSINESS' : householdId ? 'HOUSEHOLD' : undefined)}
      sidebarContextId={contextIdProp ?? businessId ?? householdId ?? undefined}
      onSidebarCreateEvent={handleSidebarCreate}
      header={
        <PageHeader
          title="Calendar"
          description={[monthLabel, contextLabel].filter(Boolean).join(' · ')}
          icon={<CalendarIcon className="h-6 w-6" />}
          actions={
            <>
              <Button variant="primary" size="sm" onClick={handleSidebarCreate}>
                <Plus className="mr-2 h-4 w-4" />
                New Event
              </Button>
              <Button variant="secondary" size="sm" onClick={() => void handleExportMonth()} title="Export month events to ICS file">
                Export
              </Button>
            </>
          }
          ...
          trailing={
            <>
              <div className="relative min-w-[200px] flex-1 sm:max-w-md">
                <input
                  className="w-full rounded-lg border border-gray-200 bg-gray-50 py-2 pl-10 pr-3 text-sm text-gray-900 focus:border-transparent focus:bg-white focus:ring-2 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 dark:focus:bg-gray-800"
              ...
              <label className="flex cursor-pointer items-center gap-2 rounded-lg bg-gray-100 px-3 py-2 transition-colors hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600">
                <input
                  type="checkbox"
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm">
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-900"
                >
    ...
    >
        ...
        {!loading && filteredEvents.length === 0 && (
          <CalendarEventsEmptyState variant="month" filtered={events.length > 0} />
        )}
        {!loading && (
          <MonthGrid 
            viewDate={viewDate} 
            events={filteredEvents} 
            onCellCreate={openCreateDrawer}
            onEventEdit={openEditDrawer}
            onEventClick={(ev) => { 
            ...
            onEventMove={async (ev, deltaDays) => {
              ...
              if (ev.recurrenceRule) {
                setPendingRecurrenceMove({ ev, payload });
                return;
              }
        ...
        return createPortal(
          ...
          >
            ...
            <div className="px-6 py-4 bg-gray-50 dark:bg-slate-800 border-t border-gray-200 dark:border-slate-700 flex items-center justify-between">
              ...
              <button
                onClick={() => openEditDrawer(eventToShow)}
...
function MonthGrid({ viewDate, events, onCellCreate, onEventClick, onEventEdit, onEventMove, myEventsOnly, currentUserId }: { viewDate: Date; events: EventItem[]; onCellCreate: (start: Date, end: Date) => void; onEventClick: (ev: EventItem) => void; onEventEdit: (ev: EventItem) => void; onEventMove: (ev: EventItem, deltaDays: number) => void; myEventsOnly: boolean; currentUserId?: string; }) {
  const { openContextMenu, contextMenu } = useCalendarEventContextMenu({
    onView: onEventClick,
    onEdit: onEventEdit,
  });
    ...
    <div className="w-full bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 overflow-hidden" onMouseUp={handleMouseUp}>
      ...
      <div className="grid grid-cols-7">
        {cells.map((cell) => (
          <button
            key={cell.idx}
            className={`relative text-left border-r border-b border-gray-200 last:border-r-0 p-2 md:p-3 min-h-[80px] md:min-h-[120px] text-xs w-full transition-all duration-200 hover:bg-gray-50 ${cell.inCurrentMonth ? 'bg-white' : 'bg-gray-50 text-gray-400'} ${cell.isToday ? 'ring-1' : ''} ${isDragging && dragStartIdx !== null && dragEndIdx !== null && cell.idx >= Math.min(dragStartIdx, dragEndIdx) && cell.idx <= Math.max(dragStartIdx, dragEndIdx) ? 'ring-2 bg-blue-50' : ''}`}
            style={cell.isToday ? {
