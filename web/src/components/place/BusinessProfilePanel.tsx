          ...
          <div className="p-5 space-y-5">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <h3 className="text-lg font-bold text-gray-900 truncate">{profile.displayName || profile.business.name}</h3>
                ...
                {profile.shortDescription && (
                  <p className="text-sm text-gray-600 mt-0.5">{profile.shortDescription}</p>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {profile.tags.map(tag => (
                <span key={tag} className="px-2.5 py-1 text-xs rounded-full bg-gray-100 text-gray-700">{tag}</span>
              ))}
            </div>
            ...
            {isFollowing && (
              <button
                ...
                className="w-full py-2 rounded-lg text-xs font-medium border border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors flex items-center justify-center gap-2"
              >
                ...
                <span className="text-gray-600">— tap to toggle</span>
              </button>
            )}
            ...
            {profile.interactionLinks.length > 0 && (
              <div className="space-y-2">
                <h4 className="text-sm font-semibold text-gray-900">Interact</h4>
                {profile.interactionLinks.map(link => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-600 truncate">{link.type.replace('_', ' ')}</p>
                    </div>
                  </a>
                ))}
              </div>
            ...
            {profile.business.description && (
              <div>
                ...
                <p className="text-sm text-gray-700">{profile.business.description}</p>
              </div>
            )}
          </div>
        )}
