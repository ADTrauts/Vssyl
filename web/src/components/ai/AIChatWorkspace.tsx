...
import AIProviderModelPicker, { type AIProvider } from './AIProviderModelPicker';
...
export type AIChatWorkspaceVariant = 'page' | 'embedded';

export interface AIChatWorkspaceProps {
  variant?: AIChatWorkspaceVariant;
  dashboardId?: string;
...
export default function AIChatWorkspace({
  ...
  dashboardType: dashboardTypeProp = 'personal',
  dashboardName: dashboardNameProp = 'Dashboard',
  ...
  const effectiveDashboardId = dashboardIdProp ?? currentDashboard?.id;
  const effectiveDashboardType = isEmbedded
    ? dashboardTypeProp
    : currentDashboard
      ? getDashboardType(currentDashboard)
      : 'personal';
  const effectiveDashboardName = isEmbedded
    ? dashboardNameProp
    : currentDashboard
  ...
  // Pre-attach files from URL (e.g. "Ask AI about this file" from Drive) — page route only
  useEffect(() => {
  ...
  }, [searchParams?.toString(), isEmbedded]);
  ...
  }, [session?.accessToken, effectiveDashboardId, ...(isEmbedded ? [] : [showArchived])]);

  // Phase 7: Load AI suggestions on mount and poll for updates (page only)
  useEffect(() => {
    if (isEmbedded || !session?.accessToken) return;
    let cancelled = false;
  ...
  }, [session?.accessToken, isEmbedded]);
  ...
  // Load available AI models for picker (page only)
  useEffect(() => {
    if (isEmbedded || !session?.accessToken) return;
  ...
  const loadConversations = async () => {
    ...
    try {
      ...
      const response = await getConversations({
        limit: 50,
        archived: isEmbedded ? false : showArchived,
        dashboardId: effectiveDashboardId,
      }, session.accessToken);
      ...
      if (response.success) {
        ...
        // Auto-select first conversation if none selected (page route only)
        if (!isEmbedded && !currentConversationId && response.data.conversations.length > 0) {
  ...
  const handleAIQuery = async (queryOverride?: string) => {
    ...
    } else if ((!queryToUse && attachedFiles.length === 0) || isAILoading) {
      return;
    }
    ...
    try {
      if (isEmbedded) {
        ...
        const twinBusinessId =
          effectiveDashboardType === 'business' &&
          currentDashboard &&
      ...
      if (extractIntent && currentAttachedFiles.length > 0) {
        ...
        if (extractRes.success && extractRes.data) {
          ...
          let conversationId = currentConversationId;
          if (!conversationId) {
            const convRes = await createConversation({ title: generateTitle(userQuery), dashboardId: effectiveDashboardId }, session.accessToken);
            if (convRes.success) {
  ...
  const handleGenerateImage = async () => {
    ...
    let conversationId = currentConversationId;
    if (!conversationId) {
      const convRes = await createConversation({ title: userContent.slice(0, 50), dashboardId: effectiveDashboardId }, session.accessToken);
      if (convRes.success) {
        conversationId = convRes.data.id;
        setCurrentConversationId(conversationId);
        loadConversations();
  ...
  const buildConversationMenuItems = useCallback(
    (conv: AIConversation): ContextMenuItem[] => [
      {
  ...
  const headerMoreMenuItems = useMemo((): ContextMenuItem[] => {
    ...
    return [
      {
        ...
        onClick: () => {
          if (currentConversationId) void handlePinConversation(currentConversationId);
        },
      },
      {
        icon: <Archive className="h-4 w-4" />,
        label: selectedConversation.isArchived ? 'Unarchive' : 'Archive',
        onClick: () => {
          if (currentConversationId) void handleArchiveConversation(currentConversationId);
        },
      },
      { divider: true },
      {
