...
function VisibilitySelector({ visibility, orgChartData, onChange }: VisibilitySelectorProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {!showAllUsers && (
        <div className="space-y-4 p-4 border border-v-border rounded-lg">
          {/* Roles */}
          {orgChartData?.roles && orgChartData.roles.length > 0 && (
            <div>
              <div className="flex items-center space-x-2 mb-2">
                <Users className="w-4 h-4 text-v-text-secondary" />
                <label className="font-medium text-sm text-v-text-secondary">Roles</label>
              </div>
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 text-sm rounded-full border-2 transition-colors ${
                      visibility.visibleToRoles?.includes(role)
                        ...
                        : 'bg-v-surface text-gray-700 border-v-border hover:border-blue-400'
                    }`}
                  >
                    {role}
                  </button>
          ...
          {orgChartData?.tiers && orgChartData.tiers.length > 0 && (
            <div>
              <div className="flex items-center space-x-2 mb-2">
                <Layers className="w-4 h-4 text-v-text-secondary" />
                <label className="font-medium text-sm text-v-text-secondary">Tiers</label>
              </div>
              <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 text-sm rounded-full border-2 transition-colors ${
                      visibility.visibleToTiers?.includes(tier)
                        ...
                        : 'bg-v-surface text-gray-700 border-v-border hover:border-green-400'
                    }`}
                  >
                    {tier}
                  </button>
          ...
          {orgChartData?.positions && orgChartData.positions.length > 0 && (
            <div>
              <div className="flex items-center space-x-2 mb-2">
                <UserCircle className="w-4 h-4 text-v-text-secondary" />
                <label className="font-medium text-sm text-v-text-secondary">Positions</label>
              </div>
              <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 text-sm rounded-full border-2 transition-colors ${
                      visibility.visibleToPositions?.includes(position.id)
                        ...
                        : 'bg-v-surface text-gray-700 border-v-border hover:border-purple-400'
                    }`}
                  >
                    {position.name}
                  </button>
          ...
          {orgChartData?.departments && orgChartData.departments.length > 0 && (
            <div>
              <div className="flex items-center space-x-2 mb-2">
                <Building2 className="w-4 h-4 text-v-text-secondary" />
                <label className="font-medium text-sm text-v-text-secondary">Departments</label>
              </div>
              <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 text-sm rounded-full border-2 transition-colors ${
                      visibility.visibleToDepartments?.includes(dept.id)
                        ...
                        : 'bg-v-surface text-gray-700 border-v-border hover:border-orange-400'
                    }`}
                  >
                    {dept.name}
                  </button>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <Card className="w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-v-border">
          ...
          <button
            onClick={onClose}
            className="p-2 hover:bg-v-surface dark:hover:bg-slate-800 dark:bg-slate-700 rounded-full transition-colors"
          >
            <X className="w-5 h-5 text-v-text-muted" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            {selectedWidgetType && (
              <p className="mt-1 text-sm text-v-text-secondary">{selectedWidgetType.description}</p>
            )}
            {errors.widgetType && (
          ...
          {/* Visibility Toggle */}
          <div className="flex items-center justify-between p-3 bg-v-background rounded-lg">
            <div>
              <label className="font-medium text-sm text-v-text-primary">Visible</label>
              <p className="text-xs text-v-text-secondary">Show this widget on the front page</p>
            </div>
            <Switch
          ...
          {/* Action Buttons */}
          <div className="flex items-center justify-end space-x-3 pt-4 border-t border-v-border">
            <Button
              type="button"
              variant="secondary"
              onClick={onClose}
