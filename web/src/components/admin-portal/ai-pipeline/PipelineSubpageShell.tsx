...
interface PipelineSubpageShellProps {
  ...
  /** Show canonical operator subnav (default true for Phase 4B intelligence sections). */
  showOperatorNav?: boolean;
  actions?: React.ReactNode;
}

export default function PipelineSubpageShell({
  title,
  description,
  children,
  showOperatorNav = true,
  actions,
}: PipelineSubpageShellProps) {
  return (
    <div className="space-y-6">
      ...
      {showOperatorNav ? <PipelineOperatorSubNav /> : null}
      {children}
    </div>
  );
}
