...
function FormGroup({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="space-y-1">
      <label className="block text-sm font-medium text-v-text-secondary">{label}</label>
      {children}
    </div>
  );
}
...
export function MemberManagement({ businessId, members, currentUserId, canManage, onMemberUpdate }: MemberManagementProps) {
  ...
  return (
    <Card>
      <div className="p-6">
        <h2 className="text-xl font-semibold text-v-text-primary mb-4">Business Members</h2>
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-v-border">
            <thead>
              <tr>
                <th className="px-4 py-2 text-left text-xs font-medium text-v-text-muted uppercase">User</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-v-text-muted uppercase">Role</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-v-text-muted uppercase">Email</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-v-text-muted uppercase">Joined</th>
                {canManage && <th className="px-4 py-2" />}
              </tr>
            </thead>
            <tbody className="bg-v-surface divide-y divide-gray-100">
              {members.map((member) => (
                <tr key={member.id}>
                  <td className="px-4 py-2 flex items-center gap-2">
                    <Avatar src={undefined} nameOrEmail={member.user.name || member.user.email} size={32} />
                    <span className="font-medium text-v-text-primary">{member.user.name || member.user.email}</span>
                  ...
                  <td className="px-4 py-2">
                    {editingMemberId === member.id ? (
                      <select
                        ...
                        className="px-2 py-1 border border-v-border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                        disabled={loading}
                      >
                      ...
                      <span className="capitalize text-v-text-secondary">{member.role.toLowerCase()}</span>
                    )}
                  </td>
                  <td className="px-4 py-2 text-v-text-secondary">{member.user.email}</td>
                  <td className="px-4 py-2 text-v-text-muted text-sm">{new Date(member.joinedAt).toLocaleDateString()}</td>
                  {canManage && (
                    <td className="px-4 py-2">
                      {editingMemberId === member.id ? (
