  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-v-text-primary">Pricing Management</h1>
          <p className="text-v-text-secondary mt-1">Manage subscription tier pricing and query pack prices</p>
        </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    <th className="text-left py-3 px-4 font-semibold text-v-text-primary">Tier</th>
                    <th className="text-left py-3 px-4 font-semibold text-v-text-primary">Monthly</th>
                    <th className="text-left py-3 px-4 font-semibold text-v-text-primary">Yearly</th>
                    <th className="text-left py-3 px-4 font-semibold text-v-text-primary">Per Employee</th>
                    <th className="text-left py-3 px-4 font-semibold text-v-text-primary">Included</th>
                    <th className="text-left py-3 px-4 font-semibold text-v-text-primary">Query Packs</th>
                    <th className="text-left py-3 px-4 font-semibold text-v-text-primary">Status</th>
                    <th className="text-left py-3 px-4 font-semibold text-v-text-primary">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {Object.entries(pricingByTier).map(([tier, prices]) => {
                    ...
                    const queryPackSummary = hasQueryPacks && m
                      ...
                      : '-';
                    return (
                    <tr key={tier} className="border-b hover:bg-v-surface-muted bg-v-surface">
                      <td className="py-3 px-4 font-medium text-v-text-primary">{formatTierName(tier)}</td>
                      <td className="py-3 px-4 text-v-text-secondary">
                      ...
                      <td className="py-3 px-4 text-v-text-secondary text-xs" title="Small / Medium / Large / Enterprise">
                        {queryPackSummary}
                      </td>
        ...
        <Tabs.Content value="history">
          <Card className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-v-text-primary">Price Change History</h2>
            ...
            {priceHistory.length === 0 ? (
              <p className="text-v-text-secondary">No price changes recorded yet</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b">
                      <th className="text-left py-3 px-4 font-semibold text-v-text-primary">Date</th>
                      <th className="text-left py-3 px-4 font-semibold text-v-text-primary">Tier</th>
                      <th className="text-left py-3 px-4 font-semibold text-v-text-primary">Change Type</th>
                      <th className="text-left py-3 px-4 font-semibold text-v-text-primary">Old Value</th>
                      <th className="text-left py-3 px-4 font-semibold text-v-text-primary">New Value</th>
                      <th className="text-left py-3 px-4 font-semibold text-v-text-primary">Changed By</th>
                      <th className="text-left py-3 px-4 font-semibold text-v-text-primary">Notification</th>
                    </tr>
                  </thead>
                  <tbody>
                    {priceHistory.map((change: any) => (
                      <tr key={change.id} className="border-b hover:bg-v-surface-muted bg-v-surface">
                        <td className="py-3 px-4 text-v-text-secondary">
          ...
          <div className="space-y-4">
            ...
            {showPreview && impact && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-4">
                <h4 className="font-semibold text-v-text-primary mb-2">Impact Preview</h4>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-v-text-secondary">Affected Subscriptions:</span>
                    <span className="font-medium text-v-text-primary">{impact.affectedSubscriptions}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-v-text-secondary">Current Monthly Revenue:</span>
                    <span className="font-medium text-v-text-primary">{formatCurrency(impact.currentRevenue)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-v-text-secondary">New Monthly Revenue:</span>
                    <span className="font-medium text-v-text-primary">{formatCurrency(impact.newRevenue)}</span>
                  </div>
                  <div className="flex justify-between border-t pt-2">
                    <span className="text-v-text-secondary">Revenue Change:</span>
                ...
                placeholder="0.00"
                  />
              <p className="text-xs text-v-text-secondary dark:text-v-text-muted mt-1">Leave empty if not applicable</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-v-text-secondary mb-1">
                    Included Employees
                ...
                placeholder="0"
                  />
              <p className="text-xs text-v-text-secondary dark:text-v-text-muted mt-1">Number of employees included in base price</p>
                </div>

            <div className="space-y-4 pt-4 border-t">
              <h4 className="text-sm font-semibold text-v-text-primary">AI Query Pack Prices</h4>
              ...
              <p className="text-xs text-v-text-secondary dark:text-v-text-muted">Query pack prices are global (same for all tiers). Changes will sync to Stripe.</p>
            </div>
            ...
            {editingPrice && editForm.basePrice !== editingPrice.basePrice && (
              <div className="space-y-3 pt-2 border-t">
                ...
                <input
                  ...
                  className="w-4 h-4"
                />
                <label htmlFor="sendNotifications" className="text-sm text-v-text-secondary">
                ...
                <div className="flex items-start gap-2">
                  ...
                  />
                  <div>
                    <label htmlFor="updateExistingSubscriptions" className="text-sm font-medium text-v-text-secondary block">
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-v-text-secondary">Add a new subscription tier. A Stripe product and monthly/yearly prices will be created and the tier will appear in the billing modal.</p>
          <div>
            <label className="block text-sm font-medium text-v-text-secondary mb-1">Tier key (slug)</label>
            <Input
              value={createTierForm.tier}
              onChange={(e) => setCreateTierForm({ ...createTierForm, tier: e.target.value })}
              placeholder="e.g. pro_plus"
            />
            <p className="text-xs text-v-text-secondary mt-1">Lowercase letters, numbers, underscores only</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-v-text-secondary mb-1">Display name</label>
            <Input
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-v-text-secondary mb-1">Monthly price ($)</label>
              <Input
                type="number"
                step="0.01"
                min="0"
            ...
            <div>
              <label className="block text-sm font-medium text-v-text-secondary mb-1">Yearly price ($)</label>
              <Input
                type="number"
                step="0.01"
                min="0"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-v-text-secondary mb-1">Per employee price ($) — optional</label>
              <Input
                type="number"
                step="0.01"
                min="0"
            ...
            <div>
              <label className="block text-sm font-medium text-v-text-secondary mb-1">Included employees — optional</label>
              <Input
                type="number"
                min="0"
