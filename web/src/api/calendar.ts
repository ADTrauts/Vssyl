...
export const calendarAPI = {
  ...
  deleteEvent: async (id: string, opts?: { editMode?: 'THIS'|'SERIES'; occurrenceStartAt?: string }) => {
    const url = new URL(`/api/calendar/events/${id}`, process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:5000');
  ...
  exportIcs: async (params: { start: string; end: string; calendarIds?: string[]; contexts?: string[] }) => {
    ...
    const url = `${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:5000'}/api/calendar/events/export?${query.toString()}`;
